function GUIMRIGraphAnalysisBUTViewMeas(fig_group,ga,bg)
...
%% Callback functions
    ...
    function update_thresholds()
        ...
        if isempty(mi)
            ...
            for i = 1:1:length(ms)
                thresholds(i) = ms{i}.getProp(MRIMeasureBUT.THRESHOLD);
            end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_threshold,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                measure_data = meas.getProp(MRIMeasureBUT.VALUES1);
            end
        end
    end
