...
%   density created from MRI cohort.
%
% See also MRIGraphAnalysisBUD, MRICohort.
...
ga = MRIGraphAnalysisBUD(cohort, Structure(), ...
    ...
    switch lower(MCF)
        ...
        case 'm'
            ...
            if Graph.isglobal(measurecode)
                ...
                disp(['Measure value = ' num2str(m.getProp(MRIMeasureBUD.VALUES1))])
                disp('=== === ===')
            end
            ...
            if Graph.isnodal(measurecode)
                ...
                values = m.getProp(MRIMeasureBUD.VALUES1);
        ...
        case 'c'
            ...
            if Graph.isglobal(measurecode)
                ...
                disp(['p-value (1) = ' num2str(c.getProp(MRIComparisonBUD.PVALUE1))])
                disp(['p-value (2) = ' num2str(c.getProp(MRIComparisonBUD.PVALUE2))])
            ...
            if Graph.isnodal(measurecode)
                ...
                values = c.diff();
                p1 = c.getProp(MRIComparisonBUD.PVALUE1);
                p2 = c.getProp(MRIComparisonBUD.PVALUE2);
                ci = c.CI(5);
        ...
        case 'r'
            ...
            if Graph.isglobal(measurecode)
                ...
                disp(['Measure code = ' int2str(measurecode) ' GLOBAL MEASURE'])
                disp(['Measure name = ' Graph.NAME{measurecode}])
                disp(['Measure value = ' num2str(mean(n.getProp(MRIRandomComparisonBUD.VALUES1)))])
                disp(['Random graph value = ' num2str(mean(n.getProp(MRIRandomComparisonBUD.RANDOM_COMP_VALUES)))])
                disp(['p-value (1) = ' num2str(n.getProp(MRIRandomComparisonBUD.PVALUE1))])
                disp(['p-value (2) = ' num2str(n.getProp(MRIRandomComparisonBUD.PVALUE2))])
            ...
            if Graph.isnodal(measurecode)
                ...
                values = n.getProp(MRIRandomComparisonBUD.VALUES1);
                disp(['Average (over regions) measure value = ' num2str(mean(values))])

                values = n.diff();
                p1 = n.getProp(MRIRandomComparisonBUD.PVALUE1);
                p2 = n.getProp(MRIRandomComparisonBUD.PVALUE2);
                ci = n.CI(5);
                disp(['Differences (per region) = ' num2str(values)])
                disp(['p-value (1) (per region) = ' num2str(p1)])
                disp(['p-value (2) (per region) = ' num2str(p2)])
