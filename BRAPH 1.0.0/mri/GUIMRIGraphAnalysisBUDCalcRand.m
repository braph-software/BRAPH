function GUIMRIGraphAnalysisBUDCalcRand(ga,mlist,selected_calc)
    ...
    function calculate(ga,mlist,g,densities,M,swaps)
        ...
        for vi = vi:1:length(densities)
            for mi = 1:1:length(mlist)
                ...
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - density = ' num2str(m.getProp(MRIMeasureBUD.DENSITY)) ' (' num2str(densities(vi)) ') - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(MRIMeasure.CODE)}];
