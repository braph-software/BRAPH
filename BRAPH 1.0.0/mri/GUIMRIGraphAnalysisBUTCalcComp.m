function GUIMRIGraphAnalysisBUTCalcComp(ga,mlist,selected_calc)
    ...
    function compare(ga,mlist,g1,g2,threshold,M)
        ...
        for vi = vi:1:length(threshold)
            for mi = 1:1:length(mlist)
               ...
               msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - threshold = ' num2str(m.getProp(MRIMeasureBUT.THRESHOLD)) ' (' num2str(threshold(vi)) ') - compare group ' int2str(g1) ' and ' int2str(g2) ' - ' Graph.NAME{m.getProp(MRIMeasureBUT.CODE)}]; %
