function GUIMRIGraphAnalysisBUTViewRand(fig_group,ga,bg)
...
%% Callback functions
    ...
    function update_thresholds()
        ...
        if isempty(ni)
            ...
            for i = 1:1:length(ns)
                thresholds(i) = ns{i}.getProp(MRIRandomComparisonBUT.THRESHOLD);
            end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_threshold,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                measure_data = meas.getProp(MRIRandomComparisonBUT.RANDOM_COMP_VALUES);
                
                fdr_lim = ones(1,length(ga.getBrainAtlas));
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = measures{value}.getProp(MRIRandomComparisonBUT.PVALUE1);
                    p2 = measures{value}.getProp(MRIRandomComparisonBUT.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
