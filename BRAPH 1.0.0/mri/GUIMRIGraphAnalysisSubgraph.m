function f = GUIMRIGraphAnalysisSubgraph(f,ga,restricted,varargin)

%% General Constants
FigName = [GUI.MGA_NAME ' : Subgraph - ' BNC.VERSION ' - ' ga.getProp(MRIGraphAnalysis.NAME)];
    ...
    function cb_menu_ga(~,~)  % (src,event)
        ...
        if ~isempty(index)
            ...
            if subgraph.length()>0
                GUIMRIGraphAnalysis(subgraph)
            end
            
        else
            msgbox('In order to create an MRI analysis subgraph should not be empty.', ...
