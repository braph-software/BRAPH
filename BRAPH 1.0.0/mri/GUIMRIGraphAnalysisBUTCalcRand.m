function GUIMRIGraphAnalysisBUTCalcRand(ga,mlist,selected_calc)
    ...
    function calculate(ga,mlist,g,threshold,M,swaps)
        ...
        for vi = vi:1:length(threshold)
            for mi = 1:1:length(mlist)
                ...
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - threshold = ' num2str(m.getProp(MRIMeasureBUT.THRESHOLD)) ' (' num2str(threshold(vi)) ') - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(MRIMeasure.CODE)}];
