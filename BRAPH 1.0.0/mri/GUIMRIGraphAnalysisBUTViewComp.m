function GUIMRIGraphAnalysisBUTViewComp(fig_group,ga,bg)
...
%% Callback functions
    ...
    function update_thresholds()
        ...
        if isempty(mi)
            ...
            for i = 1:1:length(ms)
                thresholds(i) = ms{i}.getProp(MRIComparisonBUT.THRESHOLD);
            end
        ...
        if strcmp(get(ui_list_threshold,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = measures{value}.getProp(MRIComparisonBUT.PVALUE1);
                    p2 = measures{value}.getProp(MRIComparisonBUT.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
