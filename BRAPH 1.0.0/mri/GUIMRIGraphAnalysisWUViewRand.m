function GUIMRIGraphAnalysisWUViewRand(fig_group,ga,bg)
    ...
    function update_measure_data()
        ...
        if isempty(i) || length(i)>1
            ...
            if isempty(measures)
                ...
                meas = measures{1};
                measure_data = meas.getProp(MRIRandomComparisonWU.RANDOM_COMP_VALUES);
                
                fdr_lim = ones(1,length(ga.getBrainAtlas));
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = meas.getProp(MRIRandomComparisonWU.PVALUE1);
                    p2 = meas.getProp(MRIRandomComparisonWU.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
