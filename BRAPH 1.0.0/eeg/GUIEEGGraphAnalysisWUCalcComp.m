function GUIEEGGraphAnalysisWUCalcComp(ga,mlist,selected_calc)
...
%% General Constants
APPNAME = [GUI.EGA_NAME 'weighted'];  % application name
...
%% Auxiliary functions
    ...
    function compare(ga,mlist,g1,g2,M)
        ...
        for mi = 1:1:length(mlist)
            ...
            msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - compare group ' int2str(g1) ' and ' int2str(g2) ' - ' Graph.NAME{m.getProp(EEGMeasureWU.CODE)}]; %
