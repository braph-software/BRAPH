function GUIEEGGraphAnalysisBUDCalcComp(ga,mlist,selected_calc)
    ...
    function compare(ga,mlist,g1,g2,density,M)
        ...
        for vi = vi:1:length(density)
            for mi = 1:1:length(mlist)
               ...
               msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - density = ' num2str(m.getProp(EEGMeasureBUD.DENSITY)) ' (' num2str(density(vi)) ') - compare group ' int2str(g1) ' and ' int2str(g2) ' - ' Graph.NAME{m.getProp(EEGMeasureBUD.CODE)}]; %
