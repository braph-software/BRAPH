function f = GUIEEGGraphAnalysisStructure(f,ga,restricted,varargin)

%% General Constants
FigName = [GUI.EGA_NAME ' : Community structure - ' BNC.VERSION ' - ' ga.getProp(EEGGraphAnalysis.NAME)];
...
init_table()
    function init_table()
        ...
        elseif isa(ga,'EEGGraphAnalysisBUT')
    ...
    function cd_table_calc(~,~)  % (src,event)
        ...
        elseif get(ui_checkbox_table_subject,'Value')
            ...
            subname = ga.getCohort.get(indices(subject)).getProp(EEGSubject.CODE);
    ...
    function update_popup_subject()
        ...
        if ~isempty(ind)
            ...
            for j = 1:1:length(ind)
                SubjectName{j} = ga.getCohort.get(ind(j)).getProp(EEGSubject.CODE);
            end
