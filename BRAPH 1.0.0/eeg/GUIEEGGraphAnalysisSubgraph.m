function f = GUIEEGGraphAnalysisSubgraph(f,ga,restricted,varargin)

%% General Constants
FigName = [GUI.EGA_NAME ' : Subgraph - ' BNC.VERSION ' - ' ga.getProp(EEGGraphAnalysis.NAME)];
    ...
    function cb_menu_ga(~,~)  % (src,event)
        ...
        if ~isempty(index)
            ...
            if subgraph.length()>0
                GUIEEGGraphAnalysis(subgraph)
            end
            
        else
            msgbox('In order to create an EEG analysis subgraph should not be empty.', ...
