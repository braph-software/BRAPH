function GUIEEGGraphAnalysis(tmp)
%% EEG Graph Analysis
% GUIEEGGraphAnalysis(ga) opens GUIEEGGraphAnalysis (WU, BUT or BUD)
% GUIEEGGraphAnalysis(cohort) opens cohort
% GUIEEGGraphAnalysis() opens empty analysis with empty cohort

%% General Constants
APPNAME = GUI.EGA_NAME;  % application name
BUILT = BNC.BUILT;
...
%% Application data
if exist('tmp','var') && isa(tmp,'EEGGraphAnalysis')
    ga = tmp;
    cohort = ga.getCohort();
    cs = ga.getStructure();
elseif exist('tmp','var') && isa(tmp,'EEGCohort')
    cohort = tmp;
    ga = EEGGraphAnalysisWU(cohort,Structure());
    cs = ga.getStructure();
else
...
% Callbacks to manage application data
    function cb_open(~,~)  % (src,event)
        ...
        if filterindex
            ...
            if isa(temp.ga,'EEGGraphAnalysis')
                ga = temp.ga;
                if isa(ga,'EEGGraphAnalysisWU')
                    GUIEEGGraphAnalysisWU(ga)
                elseif isa(ga,'EEGGraphAnalysisBUT')
                    GUIEEGGraphAnalysisBUT(ga)
                elseif isa(ga,'EEGGraphAnalysisBUD')
                    GUIEEGGraphAnalysisBUD(ga)
                else
                    cohort = ga.getCohort();
                    cs = ga.getStructure();
                    setup()
    ...
    function cb_import_xml(~,~)  % (scr,event)
        gatmp = update_calc(EEGCohort(BrainAtlas()));
...
COHORT_BUTTON_SELECT_TP = 'Select EEG cohort.';
COHORT_BUTTON_SELECT_MSG = 'Select file (*.ec) from where to load EEG cohort';
...
init_cohort()
    ...
    function update_cohort()
        ...
        if cohort.length>0
            set(ui_text_cohort_name,'String',cohort.getProp(EEGCohort.NAME))
            set(ui_text_cohort_repetitiontime,'String',['repetition time = ' int2str(cohort.getProp(EEGCohort.T))])
    ...
    function cb_cohort(src,~)  % (src,event)
        if strcmp(get(src,'String'),COHORT_BUTTON_VIEW_CMD)
            ...
            try
                ...
                errordlg('The file is not a valid EEG Cohort file. Please load a valid .ec file');
            end
        end
    end
...
init_calc()
    ...
    function update_calc_ganame()
        ...
        if isempty(ganame)
            set(f,'Name',[GUI.EGA_NAME ' - ' BNC.VERSION])
        else
            set(f,'Name',[GUI.EGA_NAME ' - ' BNC.VERSION ' - ' ganame])
        end
    ...
    function update_calc()
        ...
        switch graph
            case 1  % weighted undirected
                ga = EEGGraphAnalysisWU(cohort,cs, ...
                    EEGGraphAnalysis.CORR, EEGGraphAnalysis.CORR_OPTIONS{corr}, ...
                    EEGGraphAnalysis.NEG, EEGGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
            case 2  % binary undirected (fix threshold)
                ga = EEGGraphAnalysisBUT(cohort,cs, ...
                    EEGGraphAnalysis.CORR, EEGGraphAnalysis.CORR_OPTIONS{corr}, ...
                    EEGGraphAnalysis.NEG, EEGGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
            otherwise  % binary undirected (fix density)
                ga = EEGGraphAnalysisBUD(cohort,cs, ...
                    EEGGraphAnalysis.CORR, EEGGraphAnalysis.CORR_OPTIONS{corr}, ...
                    EEGGraphAnalysis.NEG, EEGGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
        end
    ...
    function cb_comm_edit(src,~)  % (src,event)
        if ~isempty(cohort.getGroup(1).getProp(Group.DATA))
            ...
            fig_structure = GUIEEGGraphAnalysisStructure(fig_structure,ga);
    ...
    function cb_calc_set(~,~)  % (src,event)
        if ga.getCohort().groupnumber()>0
            if isa(ga,'EEGGraphAnalysisWU')
                GUIEEGGraphAnalysisWU(copy(ga))
            elseif isa(ga,'EEGGraphAnalysisBUT')
                GUIEEGGraphAnalysisBUT(copy(ga))
            elseif isa(ga,'EEGGraphAnalysisBUD')
                GUIEEGGraphAnalysisBUD(copy(ga))
            end
        end
    end
    function cb_calc_sub(~,~)  % (src,event)
        if ~isempty(cohort.getGroup(1).getProp(Group.DATA))
            ...
            fig_structure = GUIEEGGraphAnalysisSubgraph(fig_structure,ga);
            waitfor(fig_structure);
