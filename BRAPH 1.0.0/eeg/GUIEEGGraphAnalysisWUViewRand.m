function GUIEEGGraphAnalysisWUViewRand(fig_group,ga,bg)
    ...
    function update_measure_data()
        ...
        if isempty(i) || length(i)>1
            ...
            if isempty(measures)
                ...
                meas = measures{1};
                measure_data = meas.getProp(EEGRandomComparisonWU.RANDOM_COMP_VALUES);
                
                fdr_lim = ones(1,length(ga.getBrainAtlas));
                
                p1 = meas.getProp(EEGRandomComparisonWU.PVALUE1);
                p2 = meas.getProp(EEGRandomComparisonWU.PVALUE2);
