function GUIEEGGraphAnalysisWUCalcMeas(ga,mlist,selected_calc)
    ...
    function calculate(ga,mlist,g)
        ...
        txt = cell(1,L);
            for mi = 1:1:length(mlist)
                ...
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(EEGMeasure.CODE)}];
