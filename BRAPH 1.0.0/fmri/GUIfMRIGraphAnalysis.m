function GUIfMRIGraphAnalysis(tmp)
%% fMRI Graph Analysis
% GUIfMRIGraphAnalysis(ga) opens GUIfMRIGraphAnalysis (WU, BUT or BUD)
% GUIfMRIGraphAnalysis(cohort) opens cohort
...
%% Application data
if exist('tmp','var') && isa(tmp,'fMRIGraphAnalysis')
    ga = tmp;
    cohort = ga.getCohort();
    cs = ga.getStructure();
elseif exist('tmp','var') && isa(tmp,'fMRICohort')
    cohort = tmp;
    ga = fMRIGraphAnalysisWU(cohort,Structure());
    cs = ga.getStructure();
else
    cohort = fMRICohort(BrainAtlas());
    ga = fMRIGraphAnalysisWU(cohort,Structure());
    cs = ga.getStructure();
end
...
% Callbacks to manage application data
    function cb_open(~,~)  % (src,event)
        ...
        if filterindex
            ...
            if isa(temp.ga,'fMRIGraphAnalysis')
                ga = temp.ga;
                if isa(ga,'fMRIGraphAnalysisWU')
                    GUIfMRIGraphAnalysisWU(ga)
                elseif isa(ga,'fMRIGraphAnalysisBUT')
                    GUIfMRIGraphAnalysisBUT(ga)
                elseif isa(ga,'fMRIGraphAnalysisBUD')
                    GUIfMRIGraphAnalysisBUD(ga)
                else
                    cohort = ga.getCohort();
                    cs = ga.getStructure();
                    setup()
    ...
    function cb_import_xml(~,~)  % (scr,event)
        gatmp = update_calc(fMRICohort(BrainAtlas()));
    ...
    function update_subjects_list()
        group = get(ui_popup_matrix_group,'value');  % selected group
        groupdata = cohort.getGroup(group).getProp(Group.DATA);
        ...
        if ~isempty(indices)
            ...
            for j = 1:1:length(indices)
                SubjectName{j} = cohort.get(indices(j)).getProp(fMRISubject.CODE);
            end
...
COHORT_BUTTON_SELECT_TP = 'Select fMRI cohort.';
COHORT_BUTTON_SELECT_MSG = 'Select file (*.fc) from where to load fMRI cohort';
...
init_cohort()
    ...
    function update_cohort()
        ...
        if cohort.length>0
            set(ui_text_cohort_name,'String',cohort.getProp(fMRICohort.NAME))
            set(ui_text_cohort_repetitiontime,'String',['repetition time = ' int2str(cohort.getProp(fMRICohort.T))])
            ...
            set(ui_text_cohort_repetitiontime,'String','repetition time = 0')
    ...
    function cb_cohort(src,~)  % (src,event)
        if strcmp(get(src,'String'),COHORT_BUTTON_VIEW_CMD)
            ...
            try
                % select file
                [file,path,filterindex] = uigetfile(GUI.fMCE_EXTENSION,GUI.fMCE_MSG_GETFILE);
                % load file
                if filterindex
                    filename = fullfile(path,file);
                    temp = load(filename,'-mat','cohort');
                    if isa(temp.cohort,'fMRICohort')
                        cohort = temp.cohort;
                        ga = fMRIGraphAnalysisWU(cohort,Structure());
                        cs = ga.getStructure();
                        setup()
                ...
                errordlg('The file is not a valid fMRI Cohort file. Please load a valid .fc file');
            end
        end
    end
...
init_calc()
    function init_calc()
        ...
        set(ui_panel_community,'Title','Set community structure')
        set(ui_panel_community,'BorderType','etchedin')
    ...
    function update_calc_ganame()
        ...
        if isempty(ganame)
            set(f,'Name',[GUI.fMGA_NAME ' - ' BNC.VERSION])
        else
            set(f,'Name',[GUI.fMGA_NAME ' - ' BNC.VERSION ' - ' ganame])
        end
    ...
    function update_calc()
        ...
        switch graph
            case 1  % weighted undirected
                ga = fMRIGraphAnalysisWU(cohort,cs, ...
                    fMRIGraphAnalysis.CORR, fMRIGraphAnalysis.CORR_OPTIONS{corr}, ...
                    fMRIGraphAnalysis.NEG, fMRIGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
            case 2  % binary undirected (fix threshold)
                ga = fMRIGraphAnalysisBUT(cohort,cs, ...
                    fMRIGraphAnalysis.CORR, fMRIGraphAnalysis.CORR_OPTIONS{corr}, ...
                    fMRIGraphAnalysis.NEG, fMRIGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
            otherwise  % binary undirected (fix density)
                ga = fMRIGraphAnalysisBUD(cohort,cs, ...
                    fMRIGraphAnalysis.CORR, fMRIGraphAnalysis.CORR_OPTIONS{corr}, ...
                    fMRIGraphAnalysis.NEG, fMRIGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
        end
    ...
    function cb_comm_edit(src,~)  % (src,event)
        if ~isempty(cohort.getGroup(1).getProp(Group.DATA))

            fig_structure = GUIfMRIGraphAnalysisStructure(fig_structure,ga);
            waitfor(fig_structure);

            update_community_info()
        else
            errordlg('Select a cohort in order to define community structure',...
    ...
    function cb_calc_set(~,~)  % (src,event)
        if ga.getCohort().groupnumber()>0
            if isa(ga,'fMRIGraphAnalysisWU')
                GUIfMRIGraphAnalysisWU(copy(ga))
            elseif isa(ga,'fMRIGraphAnalysisBUT')
                GUIfMRIGraphAnalysisBUT(copy(ga))
            elseif isa(ga,'fMRIGraphAnalysisBUD')
                GUIfMRIGraphAnalysisBUD(copy(ga))
            end
        end
    end
    function cb_calc_sub(~,~)  % (src,event)
        if ~isempty(cohort.getGroup(1).getProp(Group.DATA))
            ...
            fig_structure = GUIfMRIGraphAnalysisSubgraph(fig_structure,ga);
            waitfor(fig_structure);
...
init_matrix()
    ...
    function S = update_rearrange()
        ...
        if isa(ga,'fMRIGraphAnalysisWU')
            graph = GraphWU(A,'structure',cs);
        elseif isa(ga,'fMRIGraphAnalysisBUT')
            graph = GraphBU(A,'structure',cs,'threshold',str2num(get(ui_edit_matrix_bt,'String')));
        elseif isa(ga,'fMRIGraphAnalysisBUD')
            graph = GraphBU(A,'structure',cs,'density',str2num(get(ui_edit_matrix_bs,'String')));
        end
    ...
    function update_matrix()
        ...
        if ga.getCohort().groupnumber()>0
            ...
            groupdata = cohort.getGroup(g).getProp(Group.DATA);
            indices = find(groupdata==true);
            A = zeros(cohort.getBrainAtlas.length());
