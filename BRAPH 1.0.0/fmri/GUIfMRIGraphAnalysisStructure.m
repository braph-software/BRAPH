function f = GUIfMRIGraphAnalysisStructure(f,ga,restricted,varargin)

%% General Constants
FigName = [GUI.fMGA_NAME ' : Community structure - ' BNC.VERSION ' - ' ga.getProp(fMRIGraphAnalysis.NAME)];
...
ui_checkbox_table_group = uicontrol(ui_panel_table,'Style', 'checkbox');
ui_checkbox_table_subject = uicontrol(ui_panel_table,'Style', 'checkbox');
ui_popup_table_group = uicontrol(ui_panel_table,'Style','popup','String',{''});
ui_popup_table_subject = uicontrol(ui_panel_table,'Style','popup','String',{''});
...
init_table()
    function init_table()
        ...
        set(ui_checkbox_table_group,'Position',[.74 .16 .10 .04])
        set(ui_checkbox_table_group,'String','Group')
        set(ui_checkbox_table_group,'Value',true)
        set(ui_checkbox_table_group,'TooltipString','Calculate the community structure using the average connectivity matrix calculated of a group')
        set(ui_checkbox_table_group,'Callback',{@cd_table_groupchx})

        set(ui_checkbox_table_subject,'Position',[.84 .16 .10 .04])
        set(ui_checkbox_table_subject,'String','Subject')
        set(ui_checkbox_table_subject,'Value',false)
        set(ui_checkbox_table_subject,'TooltipString','Calculate the community structure using the connectivity matrix of a subject')
        set(ui_checkbox_table_subject,'Callback',{@cd_table_subjectchx})

        set(ui_popup_table_group,'Position',[.74 .12 .20 .04])
        set(ui_popup_table_group,'TooltipString','Select group');
        set(ui_popup_table_group,'Callback',{@cd_table_group});

        set(ui_popup_table_subject,'Position',[.74 .09 .20 .04])
        set(ui_popup_table_subject,'TooltipString','Select subject');
        set(ui_popup_table_subject,'Callback',{@cd_table_subject});
    ...
    function update_table()
        ...
        if cs.isFixed()
            ...
            set(ui_checkbox_table_group,'enable','off')
            set(ui_checkbox_table_subject,'enable','off')
            set(ui_popup_table_group,'enable','off')
            set(ui_popup_table_subject,'enable','off')
            ...
            set(ui_checkbox_table_group,'enable','on')
            set(ui_checkbox_table_subject,'enable','on')
            set(ui_popup_table_group,'enable','on')
            set(ui_popup_table_subject,'enable','on')
    ...
    function cd_table_groupchx(~,~)  % (src,event)
    ...
    function cd_table_subjectchx(~,~)  % (src,event)
        
        set(ui_checkbox_table_group,'Value',false)
        set(ui_checkbox_table_subject,'Value',true)
        
        update_table()
    end
    function cd_table_subject(~,~)  % (src,event)
        update_table()
    end
    function cd_table_group(~,~)  % (src,event)
        update_popup_subject()
        update_table()
    end
    ...
    function cd_table_calc(~,~)  % (src,event)
        ...
        groupname = ga.getCohort.getGroup(group).getProp(Group.NAME);
        indices = find(groupdata==true);

        A = zeros(ga.getCohort.getBrainAtlas.length());
        if get(ui_checkbox_table_group,'Value')
            ...
            cs.setNotes(['group:' groupname])
        elseif get(ui_checkbox_table_subject,'Value')
            
            subject = get(ui_popup_table_subject,'value');
            subname = ga.getCohort.get(indices(subject)).getProp(fMRISubject.CODE);

            if ~isempty(indices)
                A = ga.getA(indices(subject));
            end
            
            cs.setNotes(['group:' groupname ' - subject:' subname])
        end
        
        if isa(ga,'fMRIGraphAnalysisWU')
            graph = GraphWU(A,'structure',cs);
        elseif isa(ga,'fMRIGraphAnalysisBUT')
            graph = GraphBU(A,'structure',cs,'threshold',str2num(get(ui_edit_table_parameter,'String')));
        elseif isa(ga,'fMRIGraphAnalysisBUD')
            graph = GraphBU(A,'structure',cs,'density',str2num(get(ui_edit_table_parameter,'String')));
        end
...
%% Auxilary functions
    ...
    function update_popup_subject()
        group = get(ui_popup_table_group,'value');  % selected group
        groupdata = ga.getCohort.getGroup(group).getProp(Group.DATA);
        
        ind = find(groupdata==true);
        if ~isempty(ind)
            SubjectName = cell(length(ind),1);
            
            for j = 1:1:length(ind)
                SubjectName{j} = ga.getCohort.get(ind(j)).getProp(fMRISubject.CODE);
            end
            
            set(ui_popup_table_subject,'String',SubjectName)
        else
            set(ui_popup_table_subject,'String','---')
        end
    end    
