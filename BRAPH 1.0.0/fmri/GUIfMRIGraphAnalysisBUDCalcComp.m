function GUIfMRIGraphAnalysisBUDCalcComp(ga,mlist,selected_calc)
%% fMRI Graph Analysis Measure Calculator

Check.isa('Error: The fMRI graph analysis ga must be a fMRIGraphAnalysisBUD',ga,'fMRIGraphAnalysisBUD')

%% General Constants
APPNAME = [GUI.fMGA_NAME ' fixed density'];  % application name
...
%% Callback functions
    function init_compare()
        ...
        set(ui_text_selectdensity,'Position',[.49 .74 .49 .05])
...
%% Auxiliary functions
    ...
    function compare(ga,mlist,g1,g2,density,M)
        L = 100;
        txt = cell(1,L);
        for vi = vi:1:length(density)
            for mi = 1:1:length(mlist)
               
               m = ga.compare(mlist(mi),g1,g2,density(vi),'M',M,'Verbose',false,'Longitudinal',get(ui_checkbox_longitudinal,'Value'));
               msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - density = ' num2str(m.getProp(fMRIMeasureBUD.DENSITY)) ' (' num2str(density(vi)) ') - compare group ' int2str(g1) ' and ' int2str(g2) ' - ' Graph.NAME{m.getProp(fMRIMeasureBUD.CODE)}]; %
        ...
        if vi==length(density) && (isempty(mlist) || mi==length(mlist))
            txt(4:L) = txt(1:L-3);
            txt{1} = 'DONE!!!';
