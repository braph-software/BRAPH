function GUIfMRIGraphAnalysisWUCalcMeas(ga,mlist,selected_calc)
...
%% Auxiliary functions
    ...
    function calculate(ga,mlist,g)
        L = 100;
        txt = cell(1,L);
            for mi = 1:1:length(mlist)
                
                m = ga.calculate(mlist(mi),g);
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(fMRIMeasure.CODE)}];
