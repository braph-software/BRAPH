function GUIfMRIGraphAnalysisBUDViewComp(fig_group,ga,bg)

% sets position of figure
APPNAME = GUI.fMCE_NAME;
...
set(ui_list_denity,'Position',[.02 .02 .30 .30])
...
set(ui_text_density,'Position',[.02 .33 .25 .05])
...
%% Callback functions
    ...
    function update_densities()
        ...
        if isempty(mi)
            ...
            for i = 1:1:length(ms)
                densities(i) = ms{i}.getProp(fMRIComparisonBUD.DENSITY);
            end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_denity,'String'),CALC_NOTIFICATION)
            ...
            for i = 1:1:length(ga.getBrainAtlas)
                p1 = measures{value}.getProp(fMRIComparisonBUD.PVALUE1);
                p2 = measures{value}.getProp(fMRIComparisonBUD.PVALUE2);
                if get(ui_checkbox_meas_fdr1,'Value')
