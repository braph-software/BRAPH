function GUIfMRIGraphAnalysisBUDCalcRand(ga,mlist,selected_calc)
...
%% Callback functions
    function init_calculate()
        ...
        set(ui_text_selectdensity,'Position',[.49 .64 .49 .05])
...
%% Auxiliary functions
    ...
    function calculate(ga,mlist,g,densities,M,swaps)
        L = 100;
        txt = cell(1,L);
        for vi = vi:1:length(densities)
            for mi = 1:1:length(mlist)
                
                m = ga.randomcompare(mlist(mi),g,densities(vi),'M',M,'swaps',swaps,'Verbose',false);
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - density = ' num2str(m.getProp(fMRIMeasureBUD.DENSITY)) ' (' num2str(densities(vi)) ') - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(fMRIMeasure.CODE)}];
