function GUIfMRIGraphAnalysisBUD(ga)
%% fMRI Graph Analysis
% GUIfMRIGraphAnalysis(ga,true) opens analysis with only reading and basic writing permissions
% GUIfMRIGraphAnalysis(ga) opens cohort - equivalent to GUIfMRIGraphAnalysis(ga,false)
% GUIfMRIGraphAnalysis(cohort) opens empty analysis (the fMRICohort is set to cohort)
% GUIfMRIGraphAnalysis() opens empty analysis with empty cohort

%% General Constants
APPNAME = [GUI.fMGA_NAME ' BUD'];  % application name
BUILT = BNC.BUILT;
...
OPEN_SC = GUI.OPEN_SC;
OPEN_TP = ['Open fMRI graph analysis. Shortcut: ' GUI.ACCELERATOR '+' OPEN_SC];

SAVE_CMD = GUI.SAVE_CMD;
SAVE_SC = GUI.SAVE_SC;
SAVE_TP = ['Save current fMRI graph analysis. Shortcut: ' GUI.ACCELERATOR '+' SAVE_SC];

SAVEAS_CMD = GUI.SAVEAS_CMD;
SAVEAS_TP = 'Open dialog box to save current fMRI graph analysis';

IMPORT_XML_CMD = GUI.IMPORT_XML_CMD;
IMPORT_XML_TP = 'Import fMRI graph analysis from an xml file (*.xml)';

EXPORT_XML_CMD = GUI.EXPORT_XML_CMD;
EXPORT_XML_TP = 'Export fMRI graph analysis as an xml file (*.xml)';
...
%% Application data
Check.isa('Error: The fMRI graph analysis ga must be a fMRIGraphAnalysisBUD',ga,'fMRIGraphAnalysisBUD')
selected_calc = [];
...
% Callbacks to manage application data
    function cb_open(~,~)  % (src,event)
        % select file
        [file,path,filterindex] = uigetfile(GUI.fMGA_EXTENSION,GUI.fMGA_MSG_GETFILE);
        % load file
        if filterindex
            filename = fullfile(path,file);
            ga = load(filename,'-mat','ga','selected_calc','selected_brainmeasures','selected_regionmeasures','BUILT');
            if isa(ga.ga,'fMRIGraphAnalysis')
    ...
    function cb_saveas(~,~)  % (src,event)
        % select file
        [file,path,filterindex] = uiputfile(GUI.fMGA_EXTENSION,GUI.fMGA_MSG_PUTFILE);
        % save file
        if filterindex
            filename = fullfile(path,file);
            save(filename,'ga','selected_calc','selected_brainmeasures','selected_regionmeasures','BUILT');
    ...
    function cb_import_xml(~,~)  % (scr,event)
        gatmp = fMRIGraphAnalysisBUD(fMRICohort(BrainAtlas()),Structure());
    ...
    function update_subjects_list()
        ...
        if ~isempty(indices)
            ...
            for j = 1:1:length(indices)
                SubjectName{j} = ga.getCohort().get(indices(j)).getProp(fMRISubject.CODE);
            end
...
COHORT_BUTTON_VIEW_TP = ['Open fMRI cohort with ' GUI.fMCE_NAME '.'];
...
init_cohort()
    ...
    function update_cohort()
        ...
        set(ui_text_cohort_name,'String',ga.getCohort().getProp(fMRICohort.NAME))
        set(ui_text_cohort_repetitiontime,'String',['repetition time = ' int2str(ga.getCohort().getProp(fMRICohort.T))])
    ...
    function cb_cohort(~,~)  % (src,event)
        GUIfMRICohort(ga.getCohort(),true)  % open atlas with restricted permissions
    end
...
setup_calc_corr()
    function init_calc()
        ...
        set(ui_popup_calc_graph,'String',fMRIGraphAnalysis.GRAPH_OPTIONS)
        ...
        set(ui_popup_calc_corr,'String',fMRIGraphAnalysis.CORR_OPTIONS)
        ...
        set(ui_popup_calc_neg,'String',fMRIGraphAnalysis.NEG_OPTIONS)
    ...
    function update_calc_ganame()
        ganame = ga.getProp(fMRICohort.NAME);
        if isempty(ganame)
    ...
    function cb_calc_ganame(~,~)  % (src,event)
        ga.setProp(fMRIGraphAnalysis.NAME,get(ui_edit_calc_ganame,'String'));
        update_calc_ganame()
    end
    function cb_calc_new(~,~)  %  (src,event)
        GUIfMRIGraphAnalysis(copy(ga.getCohort()))
    end
    function cb_comm_view(~,~)  % (src,event)
        GUIfMRIGraphAnalysisStructure(fig_structure,ga,true);
    end
    ...
    function cb_calc_calculate(~,~)  %  (src,event)
        mlist = measurelist();
        GUIfMRIGraphAnalysisBUDCalcMeas(ga,mlist,selected_calc)
    end
    function cb_calc_compare(~,~)  %  (src,event)
        mlist = measurelist();
        GUIfMRIGraphAnalysisBUDCalcComp(ga,mlist,selected_calc)
    end
    function cb_calc_random(~,~)  %  (src,event)
        mlist = measurelist();
        GUIfMRIGraphAnalysisBUDCalcRand(ga,mlist,selected_calc)
    end
    function setup_calc_corr()
        set(ui_popup_calc_corr,'Value',find(strcmp(fMRIGraphAnalysis.CORR_OPTIONS,ga.getProp(fMRIGraphAnalysis.CORR))))
        set(ui_popup_calc_graph,'Value',find(strcmp(fMRIGraphAnalysis.GRAPH_OPTIONS,ga.getProp(fMRIGraphAnalysis.GRAPH))))
        set(ui_popup_calc_neg,'Value',find(strcmp(fMRIGraphAnalysis.NEG_OPTIONS,ga.getProp(fMRIGraphAnalysis.NEG))))
...
init_matrix()
    ...
    function S = update_rearrange()
        ...
        graph = GraphBU(A,'structure',ga.getStructure(),'density',str2num(get(ui_edit_matrix_bs,'String')));
...
BRAINMEASURES_MEAS_DENSITY_COL = 3;
BRAINMEASURES_MEAS_THRESHOLD_COL = 4;
...
BRAINMEASURES_COMP_DENSITY_COL = 9;
BRAINMEASURES_COMP_THRESHOLD1_COL = 10;
BRAINMEASURES_COMP_THRESHOLD2_COL = 11;
...
BRAINMEASURES_RAND_DENSITY_COL = 8;
BRAINMEASURES_RAND_THRESHOLD_COL = 9;
...
BRAINMEASURES_LABEL_DENSITY = 'density [%]';
...
init_brainmeasures_table()
    function init_brainmeasures_table()
        ...
        set(ui_popup_brainmeasures_comp,'Position',[.02 .06 .10 .03])
        ...
        set(ui_popup_brainmeasures_comp_group1,'Position',[.13 .06 .10 .03])
        ...
        set(ui_popup_brainmeasures_comp_group2,'Position',[.24 .06 .10 .03])
    ...
    function update_brainmeasures_table()
        ...
        if get(ui_checkbox_brainmeasures_meas,'Value')
            ...
            set(ui_table_brainmeasures,'ColumnName',{'',' value ',' density ',' threshold ',' notes ',' measure ',' param ',' group '})
            ...
            for i = 1:1:length(ms)
                if get(ui_checkbox_brainmeasures_groupavg,'Value')
                    value = mean(ms{i}.getProp(fMRIMeasureBUD.VALUES1));
                    threshold = mean(ms{i}.getProp(fMRIMeasureBUD.THRESHOLD1));
                else
                    value_temp = ms{i}.getProp(fMRIMeasureBUD.VALUES1);
                    value = value_temp(get(ui_popup_brainmeasures_subjects,'Value'));
                    
                    threshold_temp = ms{i}.getProp(fMRIMeasureBUD.THRESHOLD1);
                    threshold = threshold_temp(get(ui_popup_brainmeasures_subjects,'Value'));
                end
                ...
                data{i,BRAINMEASURES_MEAS_THRESHOLD_COL} = threshold;
                data{i,BRAINMEASURES_MEAS_DENSITY_COL} = ms{i}.getProp(fMRIMeasureBUD.DENSITY);
                data{i,BRAINMEASURES_MEAS_NOTES_COL} = ms{i}.getProp(fMRIMeasureBUD.NOTES);
                data{i,BRAINMEASURES_MEAS_MEASURE_COL} = Graph.NAME{ms{i}.getProp(fMRIMeasureBUD.CODE)};
                data{i,BRAINMEASURES_MEAS_PARAM_COL} = ms{i}.getProp(fMRIMeasureBUD.PARAM);
                data{i,BRAINMEASURES_MEAS_GROUP_COL} = ms{i}.getProp(fMRIMeasureBUD.GROUP1);
            end
        ...
        elseif get(ui_checkbox_brainmeasures_comp,'Value')
            ...
            set(ui_table_brainmeasures,'ColumnName',{'',' difference ',' p (1-tailed) ',' p (2-tailed) ',' value 1 ',' value 2 ',' CI lower ',' CI upper ',' density ',' threshold 1 ',' threshold 2 ',' notes ',' measure ',' param ',' group 1 ',' group 2 '})
            ...
            for i = 1:1:length(cs)
                ...
                data{i,BRAINMEASURES_COMP_PVALUE1_COL} = cs{i}.getProp(fMRIComparisonBUD.PVALUE1);
                data{i,BRAINMEASURES_COMP_PVALUE2_COL} = cs{i}.getProp(fMRIComparisonBUD.PVALUE2);
                data{i,BRAINMEASURES_COMP_VALUE1_COL} = cs{i}.getProp(fMRIComparisonBUD.VALUES1);
                data{i,BRAINMEASURES_COMP_VALUE2_COL} = cs{i}.getProp(fMRIComparisonBUD.VALUES2);
                
                confint = cs{i}.CI(5);
                data{i,BRAINMEASURES_COMP_CIL_COL} = confint(1);
                data{i,BRAINMEASURES_COMP_CIU_COL} = confint(2);
                
                density = cs{i}.getProp(fMRIComparisonBUD.DENSITY);
                data{i,BRAINMEASURES_COMP_DENSITY_COL} = density;
                
                threshold1 = cs{i}.getProp(fMRIComparisonBUD.THRESHOLD1);
                threshold2 = cs{i}.getProp(fMRIComparisonBUD.THRESHOLD2);
                data{i,BRAINMEASURES_COMP_THRESHOLD1_COL} = mean(threshold1);
                data{i,BRAINMEASURES_COMP_THRESHOLD2_COL} = mean(threshold2);
                
                data{i,BRAINMEASURES_COMP_NOTES_COL} = cs{i}.getProp(fMRIComparisonBUD.NOTES);
                data{i,BRAINMEASURES_COMP_MEASURE_COL} = Graph.NAME{cs{i}.getProp(fMRIComparisonBUD.CODE)};
                data{i,BRAINMEASURES_COMP_PARAM_COL} = cs{i}.getProp(fMRIComparisonBUD.PARAM);
                data{i,BRAINMEASURES_COMP_GROUP1_COL} = cs{i}.getProp(fMRIComparisonBUD.GROUP1);
                data{i,BRAINMEASURES_COMP_GROUP2_COL} = cs{i}.getProp(fMRIComparisonBUD.GROUP2);
            end
        ...
        elseif get(ui_checkbox_brainmeasures_rand,'Value')
            ...
            set(ui_table_brainmeasures,'ColumnName',{'',' comp value ',' p (1-tailed) ',' p (2-tailed) ',' real value ',' CI lower ',' CI upper ',' density ',' threshold ',' notes ',' measure ',' param ',' group '})
            ...
            for i = 1:1:length(ns)
                ...
                ns{i}.getProp(fMRIRandomComparisonBUD.RANDOM_COMP_VALUES);
                data{i,BRAINMEASURES_RAND_COMPVALUE_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.RANDOM_COMP_VALUES);
                data{i,BRAINMEASURES_RAND_PVALUE1_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.PVALUE1);
                data{i,BRAINMEASURES_RAND_PVALUE2_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.PVALUE2);
                data{i,BRAINMEASURES_RAND_REALVALUE_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.VALUES1);
                
                confint = ns{i}.CI(5);
                data{i,BRAINMEASURES_RAND_CIL_COL} = confint(1);
                data{i,BRAINMEASURES_RAND_CIU_COL} = confint(2);
                
                density = ns{i}.getProp(fMRIRandomComparisonBUD.DENSITY);
                data{i,BRAINMEASURES_RAND_DENSITY_COL} = density;
                
                threshold = ns{i}.getProp(fMRIRandomComparisonBUD.THRESHOLD1);
                data{i,BRAINMEASURES_RAND_THRESHOLD_COL} = mean(threshold);
                
                data{i,BRAINMEASURES_RAND_NOTES_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.NOTES);
                data{i,BRAINMEASURES_RAND_MEASURE_COL} = Graph.NAME{ns{i}.getProp(fMRIRandomComparisonBUD.CODE)};
                data{i,BRAINMEASURES_RAND_PARAM_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.PARAM);
                data{i,BRAINMEASURES_RAND_GROUP_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.GROUP1);
            end
            ...
            if get(ui_checkbox_brainmeasures_holdon,'Value')
                ...
                if get(ui_checkbox_brainmeasures_meas,'Value')
                    ...
                    if ~isempty(mi)
                        ...
                        str3 = get(ui_popup_brainmeasures_subjects,'String');
                        
                        if get(ui_checkbox_brainmeasures_groupavg,'Value')
                            name = ['measure - ' str1{get(ui_popup_brainmeasures_comp,'Value')} ' - ' 'group = ' int2str(get(ui_popup_brainmeasures_comp_group1,'Value'))];
                        else
                            name = ['measure - ' str1{get(ui_popup_brainmeasures_comp,'Value')} ' - ' ' subject ' str3{get(ui_popup_brainmeasures_subjects,'Value')} ' - ' 'group = ' int2str(get(ui_popup_brainmeasures_comp_group1,'Value'))];
                        end
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ms)
                                ...
                                meas(i) = value;
                                d(i) = ms{i}.getProp(fMRIMeasureBUD.DENSITY);
                            end
                            ...
                            xlabel(BRAINMEASURES_LABEL_DENSITY)
                ...
                elseif get(ui_checkbox_brainmeasures_comp,'Value')
                    ...
                    if ~isempty(ci)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(cs)
                                diff(i) = cs{i}.diff();
                                d(i) = cs{i}.getProp(fMRIMeasureBUD.DENSITY);
                                m(i) = cs{i}.CI(50);
                                ci5 = cs{i}.CI(5);
                                ci5l(i) = ci5(1);
                                ci5u(i) = ci5(2);
                ...
                elseif get(ui_checkbox_brainmeasures_rand,'Value')
                    ...
                    if ~isempty(ni)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ns)
                                meas(i) = ns{i}.getProp(fMRIRandomComparisonBUD.RANDOM_COMP_VALUES);
                                d(i) = ns{i}.getProp(fMRIRandomComparisonBUD.DENSITY);
                                m(i) = ns{i}.CI(50);
                                ci5 = ns{i}.CI(5);
                                ci5l(i) = ci5(1);
                                ci5u(i) = ci5(2);
...
REGIONMEASURES_MEAS_DENSITY_COL = 3;
REGIONMEASURES_MEAS_THRESHOLD_COL = 4;
...
REGIONMEASURES_COMP_DENSITY_COL = 11;
REGIONMEASURES_COMP_THRESHOLD1_COL = 12;
REGIONMEASURES_COMP_THRESHOLD2_COL = 13;
...
REGIONMEASURES_RAND_DENSITY_COL = 10;
REGIONMEASURES_RAND_THRESHOLD_COL = 11;
...
REGIONMEASURES_LABEL_DENSITY = 'density [%]';
REGIONMEASURES_LABEL_THRESHOLD = 'threshold [n.u.]';
...
init_regionmeasures()
    function init_regionmeasures()
        ...
        set(ui_popup_regionmeasures_comp,'Position',[.02 .06 .08 .03])
        ...
        set(ui_popup_regionmeasures_comp_br,'Position',[.11 .06 .07 .03])
        ...
        set(ui_popup_regionmeasures_comp_group1,'Position',[.19 .06 .07 .03])
        ...
        set(ui_popup_regionmeasures_comp_group2,'Position',[.27 .06 .07 .03])
    ...
    function update_regionmeasures_table()
        ...
        if get(ui_checkbox_regionmeasures_meas,'Value')
            ...
            set(ui_table_regionmeasures,'ColumnName',{'',' value ',' density ',' threshold ',' notes ',' measure ',' group ',' brain region ',' param '})
            ...
            for i = 1:1:length(ms)
                value_tmp = ms{i}.getProp(fMRIMeasureBUD.VALUES1);
                ...
                data{i,REGIONMEASURES_MEAS_THRESHOLD_COL} = threshold;
                data{i,REGIONMEASURES_MEAS_DENSITY_COL} = ms{i}.getProp(fMRIMeasureBUD.DENSITY);
                data{i,REGIONMEASURES_MEAS_NOTES_COL} = ms{i}.getProp(fMRIMeasureBUD.NOTES);
                data{i,REGIONMEASURES_MEAS_MEASURE_COL} = Graph.NAME{ms{i}.getProp(fMRIMeasureBUD.CODE)};
                data{i,REGIONMEASURES_MEAS_GROUP_COL} = ms{i}.getProp(fMRIMeasureBUD.GROUP1);
                data{i,REGIONMEASURES_MEAS_BR_COL} = BrainRegions{bri};
                data{i,REGIONMEASURES_MEAS_PARAM_COL} = ms{i}.getProp(fMRIMeasureBUD.PARAM);
            end
        ...
        elseif get(ui_checkbox_regionmeasures_comp,'Value')
            ...
            set(ui_table_regionmeasures,'ColumnName',{'',' difference ',' p (1-tailed) ',' fdr (1-tailed) ',' p (2-tailed) ',' fdr (2-tailed) ',' value 1 ',' value 2 ',' CI lower ',' CI upper ',' density 1 ',' threshold 1 ',' threshold 2 ',' notes ',' measure ',' param ',' group 1 ',' group 2 ',' brainregion '})
            ...
            for i = 1:1:length(cs)
                ...
                density = cs{i}.getProp(fMRIComparisonBUD.DENSITY);
                data{i,REGIONMEASURES_COMP_DENSITY_COL} = density;
                
                threshold1 = cs{i}.getProp(fMRIComparisonBUD.THRESHOLD1);
                threshold2 = cs{i}.getProp(fMRIComparisonBUD.THRESHOLD2);
                data{i,REGIONMEASURES_COMP_THRESHOLD1_COL} = mean(threshold1);
                data{i,REGIONMEASURES_COMP_THRESHOLD2_COL} = mean(threshold2);
                
                data{i,REGIONMEASURES_COMP_NOTES_COL} = cs{i}.getProp(fMRIComparisonBUD.NOTES);
                data{i,REGIONMEASURES_COMP_MEASURE_COL} = Graph.NAME{cs{i}.getProp(fMRIComparisonBUD.CODE)};
                data{i,REGIONMEASURES_COMP_PARAM_COL} = cs{i}.getProp(fMRIComparisonBUD.PARAM);
                data{i,REGIONMEASURES_COMP_GROUP1_COL} = cs{i}.getProp(fMRIComparisonBUD.GROUP1);
                data{i,REGIONMEASURES_COMP_GROUP2_COL} = cs{i}.getProp(fMRIComparisonBUD.GROUP2);
        ...
        elseif get(ui_checkbox_regionmeasures_rand,'Value')
            ...
            set(ui_table_regionmeasures,'ColumnName',{'',' comp value ',' p (1-tailed) ',' fdr (1-tailed) ',' p (2-tailed) ',' fdr (2-tailed) ',' real value ',' CI lower ',' CI upper ',' density ',' threshold ',' notes ',' measure ',' param ',' group ',' brainregion '})
            ...
            for i = 1:1:length(ns)
                ...
                threshold = ns{i}.getProp(fMRIRandomComparisonBUD.THRESHOLD1);
                data{i,REGIONMEASURES_RAND_THRESHOLD_COL} = mean(threshold);
                
                data{i,REGIONMEASURES_RAND_NOTES_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.NOTES);
                data{i,REGIONMEASURES_RAND_MEASURE_COL} = Graph.NAME{ns{i}.getProp(fMRIRandomComparisonBUD.CODE)};
                data{i,REGIONMEASURES_RAND_PARAM_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.PARAM);
                data{i,REGIONMEASURES_RAND_GROUP_COL} = ns{i}.getProp(fMRIRandomComparisonBUD.GROUP1);
            ...
            if get(ui_checkbox_regionmeasures_holdon,'Value')
                ...
                if get(ui_checkbox_regionmeasures_meas,'Value')
                    ...
                    if ~isempty(mi)
                        ...
                        str3 = get(ui_popup_regionmeasures_subjects,'String');
                        ...
                        if get(ui_checkbox_regionmeasures_groupavg,'Value')
                            ...
                            name = ['br.region - ' br_name ' measure - ' str1{get(ui_popup_regionmeasures_comp,'Value')} ' - ' ' subject ' str3{get(ui_popup_regionmeasures_subjects,'Value')} ' - ' 'group = ' int2str(get(ui_popup_regionmeasures_comp_group1,'Value'))];
                        end
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            xlabel(REGIONMEASURES_LABEL_DENSITY)
                    ...
                    if ~isempty(ni)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ns)
                                ...
                                temp = ns{i}.getProp(fMRIRandomComparisonBUD.RANDOM_COMP_VALUES);
                                meas(i) = temp(bri);
...
init_brainview()
    ...
    function cb_brainview_viewgroup(~,~)  % (src,event)
        ...
        if ga.getCohort().length()>0
            GUIfMRIGraphAnalysisBUDViewMeas(fig_group,ga,bg)
        else
    ...
    function cb_brainview_viewcomparison(~,~)  % (src,event)
        ...
        if ga.getCohort().length()>0
            GUIfMRIGraphAnalysisBUDViewComp(fig_group,ga,bg)
        else
    ...
    function cb_brainview_viewrandom(~,~)  % (src,event)
        ...
        if ga.getCohort().length()>0
            GUIfMRIGraphAnalysisBUDViewRand(fig_group,ga,bg)
        else
    ...
    function cb_brainview_graph(~,~)  % (src,event)
        ...
        GUIfMRIGraphAnalysisBUDSetGraph(f,ga,bg)
    end
...
init_menu()
    ...
    function cb_menu_figure(~,~)  % (src,event)
        ...
        elseif strcmpi(get(ui_panel_brainmeasures,'Visible'),'on')
            ...
            if get(ui_checkbox_brainmeasures_meas,'Value')
                h = figure('Name', [Graph.NAME{mlist(get(ui_popup_brainmeasures_meas,'Value'))} ' - ' ga.getCohort().getGroup(get(ui_popup_brainmeasures_meas_group,'Value')).getProp(Group.NAME)]);
            elseif get(ui_checkbox_brainmeasures_comp,'Value')
        ...
        elseif strcmpi(get(ui_panel_regionmeasures,'Visible'),'on')
            ...
            if get(ui_checkbox_regionmeasures_meas,'Value')
                h = figure('Name', [Graph.NAME{mlist(get(ui_popup_regionmeasures_meas,'Value'))} ' - ' ga.getCohort().getGroup(get(ui_popup_regionmeasures_meas_group,'Value')).getProp(Group.NAME)]);
            elseif get(ui_checkbox_regionmeasures_comp,'Value')
                h = figure('Name', [Graph.NAME{mlist(get(ui_popup_brainmeasures_comp,'Value'))} ' - ' ga.getCohort().getGroup(get(ui_popup_regionmeasures_comp_group1,'Value')).getProp(Group.NAME) ' vs. ' ga.getCohort().getGroup(get(ui_popup_regionmeasures_comp_group2,'Value')).getProp(Group.NAME)]);
            end
            set(gcf,'color','w')
