function GUIfMRIGraphAnalysisBUTCalcMeas(ga,mlist,selected_calc)
...
%% Callback functions
    function init_calculate()
        ...
        set(ui_text_selectthreshold,'Position',[.49 .85 .49 .05])
...
%% Auxiliary functions
    ...
    function calculate(ga,mlist,g,threshold)
        L = 100;
        txt = cell(1,L);
        for vi = vi:1:length(threshold)
            for mi = 1:1:length(mlist)
                
                m = ga.calculate(mlist(mi),g,threshold(vi));
