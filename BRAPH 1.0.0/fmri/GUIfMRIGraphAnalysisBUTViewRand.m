function GUIfMRIGraphAnalysisBUTViewRand(fig_group,ga,bg)
...
set(ui_list_threshold,'Position',[.02 .02 .30 .40])
...
set(ui_text_threshold,'Position',[.02 .4275 .25 .05])
...
%% Callback functions
    ...
    function update_thresholds()
        ...
        if isempty(ni)
            ...
            thresholds = zeros(1,length(ns));
            for i = 1:1:length(ns)
                thresholds(i) = ns{i}.getProp(fMRIRandomComparisonBUT.THRESHOLD);
            end
            set(ui_list_threshold,'String',thresholds)
        end
    end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_threshold,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                measure_data = meas.getProp(fMRIRandomComparisonBUT.RANDOM_COMP_VALUES);
                
                fdr_lim = ones(1,length(ga.getBrainAtlas));
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = measures{value}.getProp(fMRIRandomComparisonBUT.PVALUE1);
                    p2 = measures{value}.getProp(fMRIRandomComparisonBUT.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
