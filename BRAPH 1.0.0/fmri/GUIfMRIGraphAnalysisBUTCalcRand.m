function GUIfMRIGraphAnalysisBUTCalcRand(ga,mlist,selected_calc)
...
%% General Constants
APPNAME = [GUI.fMGA_NAME 'fixed threshold'];  % application name
...
%% Callback functions
    function init_calculate()
        ...
        set(ui_text_selectthreshold,'Position',[.49 .64 .49 .05])
        ...
        set(ui_text_info,'String',ga.getProp(fMRIGraphAnalysis.NAME))
        ...
        if strcmp(get(src,'String'),'Calculate measures')
            if ga.getCohort().groupnumber()>0
                if isempty(selected_calc)
                    ...
                    calculate(ga,mlist,g,values,M,swaps)
                end
            else
...
%% Auxiliary functions
    ...
    function calculate(ga,mlist,g,threshold,M,swaps)
        L = 100;
        txt = cell(1,L);
        for vi = vi:1:length(threshold)
            for mi = 1:1:length(mlist)
                
                m = ga.randomcompare(mlist(mi),g,threshold(vi),'M',M,'swaps',swaps,'Verbose',false);
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - threshold = ' num2str(m.getProp(fMRIMeasureBUT.THRESHOLD)) ' (' num2str(threshold(vi)) ') - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(fMRIMeasure.CODE)}];
