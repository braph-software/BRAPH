function GUIfMRICohort(tmp,restricted)
%% fMRI Cohort Editor
...
%% General Constants
APPNAME = GUI.fMCE_NAME;  % application name
BUILT = BNC.BUILT;
...
OPEN_SC = GUI.OPEN_SC;
OPEN_TP = ['Open fMRI cohort. Shortcut: ' GUI.ACCELERATOR '+' OPEN_SC];

SAVE_CMD = GUI.SAVE_CMD;
SAVE_SC = GUI.SAVE_SC;
SAVE_TP = ['Save current fMRI cohort. Shortcut: ' GUI.ACCELERATOR '+' SAVE_SC];

SAVEAS_CMD = GUI.SAVEAS_CMD;
SAVEAS_TP = 'Open dialog box to save current fMRI cohort';

IMPORT_XML_CMD = GUI.IMPORT_XML_CMD;
IMPORT_XML_TP = 'Import fMRI cohort from an xml file (*.xml)';

EXPORT_XML_CMD = GUI.EXPORT_XML_CMD;
EXPORT_XML_TP = 'Export fMRI cohort as an xml file (*.xml)';
...
fMRIGRAPHANALYSIS_CMD = 'New fMRI graph analysis ...';
fMRIGRAPHANALYSIS_TP = ['Generate new fMRI graph analysis and opens it with ' GUI.fMGA_NAME];

%% Application data
if exist('tmp','var') && isa(tmp,'fMRICohort')
    cohort = tmp;
elseif exist('tmp','var') && isa(tmp,'BrainAtlas')
    cohort = fMRICohort(tmp);
else
...
% Callbacks to manage application data
    ...
    function cb_saveas(~,~)  % (src,event)
        % select file
        [file,path,filterindex] = uiputfile(GUI.fMCE_EXTENSION,GUI.fMCE_MSG_PUTFILE);
        % save file
        if filterindex
            filename = fullfile(path,file);
            save(filename,'cohort','selected_group','selected_subjects','BUILT');
    ...
    function cb_import_xml(~,~)  % (scr,event)
        cohorttmp = fMRICohort(BrainAtlas());
...
init_atlas()
    ...
    function update_atlas()
        if cohort.getBrainAtlas().length>0
            set(ui_text_atlas_name,'String',cohort.getBrainAtlas().getProp(fMRICohort.NAME))
...
GRTAB_RT_TP = 'Repetition time of fMRI in seconds';
...
GRTAB_LOAD_XML_TP = 'Load subjects from another fMRI cohort XML file. All subjects will be in the same (new) group';
...
init_grtab()
    ...
    function update_grtab_cohortname()
        cohortname = cohort.getProp(fMRICohort.NAME);
        if isempty(cohortname)
            set(f,'Name',[GUI.fMCE_NAME ' - ' BNC.VERSION])
        else
            set(f,'Name',[GUI.fMCE_NAME ' - ' BNC.VERSION ' - ' cohortname])
        end
    ...
    function cb_grtab_cohortname(~,~)  % (src,event)
        cohort.setProp(fMRICohort.NAME,get(ui_edit_grtab_cohortname,'String'));
    ...
    function cb_edit_time(~,~)  % (src,event)
        ...
        cohort.setProp(fMRICohort.T,time)
    end
    function cb_grtab_load_xls(~,~)  % (src,event)
        try
            cohorttmp = fMRICohort(cohort.getBrainAtlas());
            ...
            errordlg('The file is not a valid fMRI Subjects file. Please load a valid Excel file');
        end
    end
    function cb_grtab_load_mat(~,~)  % (src,event)
        try
            ...
            errordlg('The file is not a valid fMRI Subjects file. Please load a valid MAT file');
        end
    end
    function cb_grtab_load_xml(~,~)  % (src,event)
        try
            ...
            errordlg('The file is not a valid fMRI Subjects file. Please load a valid Xml file');
        end
    end
...
init_groups()
    ...
    function update_groups(varargin)
        ...
        ColumnFormat = {'logical','char','numeric',fMRISubject.GENDER_OPTIONS,'char'};
        for g = 1:1:cohort.groupnumber()
        ...
        for i = 1:1:cohort.length()
            ...
            data{i,TAB_GROUPS_SUBCODE_COL} = cohort.get(i).getProp(fMRISubject.CODE);
            data{i,TAB_GROUPS_SUBAGE_COL} = cohort.get(i).getProp(fMRISubject.AGE);
            data{i,TAB_GROUPS_SUBGENDER_COL} = cohort.get(i).getProp(fMRISubject.GENDER);
            data{i,TAB_GROUPS_SUBNOTES_COL} = cohort.get(i).getProp(fMRISubject.NOTES);
        end
        for g = 1:1:cohort.groupnumber()
    ...
    function cb_groups_edit_sub(~,event)  % (src,event)
        ...
        switch col
            ...
            case TAB_GROUPS_SUBCODE_COL
                cohort.get(i).setProp(fMRISubject.CODE,newdata)
            case TAB_GROUPS_SUBAGE_COL
                cohort.get(i).setProp(fMRISubject.AGE,newdata)
            case TAB_GROUPS_SUBGENDER_COL
                cohort.get(i).setProp(fMRISubject.GENDER,newdata)
            case TAB_GROUPS_SUBNOTES_COL
                cohort.get(i).setProp(fMRISubject.NOTES,newdata)
            otherwise
                g = col-5;
...
%         set(ui_button_subjects_save_txt,'Callback',{@cb_subjects_save_txt})
        ...
        set(ui_list_subjects,'TooltipString','Select fMRI subject');
    ...
    function update_subjects_table(varargin)
        ...
        if ~isempty(indices)
            if size(cohort.get(indices(value)).getProp(fMRISubject.DATA),2) ~= cohort.getBrainAtlas.length()
                cohort.get(indices(value)).setProp(fMRISubject.DATA,zeros(1,cohort.getBrainAtlas.length()))
            end
            data = cohort.get(indices(value)).getProp(fMRISubject.DATA);
    ...
    function cb_subjects_edit_data(~,event)  % (src,event)
        ...
        data(j,col) = newdata;
        cohort.get(indices(value)).setProp(fMRISubject.DATA,data);
    ...
    function cb_subjects_add_row(~,~)  % (src,event)
        ...
        if ~isempty(indices)
            ...
            cohort.get(indices(value)).setProp(fMRISubject.DATA,data);
            cohort.get(indices(value)).getProp(fMRISubject.DATA);
...
init_menu()
    function init_menu()
        ...
        set(ui_menu_analysis_ga,'Label',fMRIGRAPHANALYSIS_CMD)
        set(ui_menu_analysis_ga,'Callback',{@cb_menu_ga})
    end
    function cb_menu_ga(~,~)  % (src,event)
        if cohort.length()>0
            GUIfMRIGraphAnalysis(cohort.copy())
        else
            msgbox('In order to create an fMRI analysis the cohort must have at least one subject.', ...
                'Warning: Empty fMRI cohort', ...
                'warn')
        end
    end
