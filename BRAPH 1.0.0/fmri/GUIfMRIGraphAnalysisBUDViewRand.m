function GUIfMRIGraphAnalysisBUDViewRand(fig_group,ga,bg)

% sets position of figure
APPNAME = GUI.fMGA_NAME;
...
set(ui_list_gr,'Position',[.02 .515 .30 .35])
set(ui_list_gr,'TooltipString','Select brain regions');
set(ui_list_gr,'Callback',{@cb_list_gr});

ui_list_denity = uicontrol(fig_group,'Style', 'listbox');
set(ui_list_denity,'Units','normalized')
set(ui_list_denity,'BackgroundColor',GUI.BKGCOLOR)
set(ui_list_denity,'String','Select measure to view applicable densities')
set(ui_list_denity,'Value',1)
set(ui_list_denity,'Max',-1,'Min',0)
set(ui_list_denity,'BackgroundColor',[1 1 1])
set(ui_list_denity,'Position',[.02 .02 .30 .40])
set(ui_list_denity,'TooltipString','Select density');
set(ui_list_denity,'Callback',{@cb_list_density});

ui_text_density = uicontrol(fig_group,'Style','text');
set(ui_text_density,'Units','normalized')
set(ui_text_density,'String','Select density %')
set(ui_text_density,'Position',[.02 .4275 .25 .05])
set(ui_text_density,'HorizontalAlignment','left')
set(ui_text_density,'FontWeight','bold')
...
%% Callback functions
    ...
    function update_densities()
        [ns,ni] = ga.getRandomComparisons(mlist(get(ui_list_gr,'Value')),get(ui_popup_grouplists,'Value'));
        if isempty(ni)
            set(ui_list_denity,'String',CALC_NOTIFICATION)
            set(ui_list_denity,'Value',1)
        else
            densities = zeros(1,length(ns));
            for i = 1:1:length(ns)
                densities(i) = ns{i}.getProp(fMRIRandomComparisonBUD.DENSITY);
            end
            set(ui_list_denity,'String',densities)
        end
    end
    ...
    function cb_list_density(~,~)  % (src,event)
        update_densities()
        update_measure_data()
        update_brain_meas_plot()        
    end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_denity,'String'),CALC_NOTIFICATION)
            errordlg('The measure is not yet calculated')
        else
            value = get(ui_list_denity,'Value');
            [measures,mi] = ga.getRandomComparisons(mlist(get(ui_list_gr,'Value')),get(ui_popup_grouplists,'Value'));
            meas = measures{1,value};
            if isempty(meas)
                errordlg('The measure does not exist for the selected density')
            else
                measure_data = meas.getProp(fMRIRandomComparisonBUD.RANDOM_COMP_VALUES);
            end
            
            fdr_lim = ones(1,length(ga.getBrainAtlas));
            for i = 1:1:length(ga.getBrainAtlas)
                p1 = measures{value}.getProp(fMRIRandomComparisonBUD.PVALUE1);
                p2 = measures{value}.getProp(fMRIRandomComparisonBUD.PVALUE2);
                if get(ui_checkbox_meas_fdr1,'Value')
