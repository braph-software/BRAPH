function GUIfMRIGraphAnalysisBUDViewMeas(fig_group,ga,bg)
...
set(ui_list_denity,'TooltipString','Select brain regions');
...
%% Callback functions
    ...
    function update_densities()
        [ms,mi] = ga.getMeasures(mlist(get(ui_list_gr,'Value')),get(ui_popup_grouplists,'Value'));
        if isempty(mi)
            set(ui_list_denity,'String',CALC_NOTIFICATION)
            set(ui_list_denity,'Value',1)
        else
            densities = zeros(1,length(ms));
            for i = 1:1:length(ms)
                densities(i) = ms{i}.getProp(fMRIMeasureBUD.DENSITY);
            end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_denity,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                measure_data_tmp = meas.getProp(fMRIMeasureBUD.VALUES1);
                measure_data = mean(measure_data_tmp);
            end
        end
    end
