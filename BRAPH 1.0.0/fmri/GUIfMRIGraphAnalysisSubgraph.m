function f = GUIfMRIGraphAnalysisSubgraph(f,ga,restricted,varargin)

%% General Constants
FigName = [GUI.fMGA_NAME ' : Subgraph - ' BNC.VERSION ' - ' ga.getProp(fMRIGraphAnalysis.NAME)];
    ...
    function cb_menu_ga(~,~)  % (src,event)
        ...
        if ~isempty(index)
            ...
            if subgraph.length()>0
                GUIfMRIGraphAnalysis(subgraph)
            end
            
        else
            msgbox('In order to create an fMRI analysis subgraph should not be empty.', ...
