function GUIfMRIGraphAnalysisWUViewRand(fig_group,ga,bg)
...
%% Callback functions
    ...
    function update_measure_data()
        ...
        if isempty(i) || length(i)>1
            ...
            [measures,~] = ga.getRandomComparisons(mlist(get(ui_list_gr,'Value')),get(ui_popup_grouplists,'Value'));
            if isempty(measures)
                errordlg('The measure does not exist')
            else
                meas = measures{1};
                measure_data = meas.getProp(fMRIRandomComparisonWU.RANDOM_COMP_VALUES);
                
                fdr_lim = ones(1,length(ga.getBrainAtlas));
                
                p1 = meas.getProp(fMRIRandomComparisonWU.PVALUE1);
                p2 = meas.getProp(fMRIRandomComparisonWU.PVALUE2);
