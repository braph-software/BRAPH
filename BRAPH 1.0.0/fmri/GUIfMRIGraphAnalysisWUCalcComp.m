function GUIfMRIGraphAnalysisWUCalcComp(ga,mlist,selected_calc)
...
%% General Constants
APPNAME = [GUI.fMGA_NAME 'weighted'];  % application name
...
%% Callback functions
    function init_compare()
        ...
        set(ui_checkbox_longitudinal,'Position',[.75 .70 .23 .08])
        ...
        set(ui_edit_info,'Position',[.49 .15 .49 .45])
...
%% Auxiliary functions
    ...
    function compare(ga,mlist,g1,g2,M)
        L = 100;
        txt = cell(1,L);
        
        for mi = 1:1:length(mlist)
            
            m = ga.compare(mlist(mi),g1,g2,'M',M,'Verbose',false,'Longitudinal',get(ui_checkbox_longitudinal,'Value'));
            msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - compare group ' int2str(g1) ' and ' int2str(g2) ' - ' Graph.NAME{m.getProp(fMRIMeasureWU.CODE)}]; %
