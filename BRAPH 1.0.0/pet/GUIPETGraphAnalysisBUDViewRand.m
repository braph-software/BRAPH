function GUIPETGraphAnalysisBUDViewRand(fig_group,ga,bg)
...
%% Callback functions
    ...
    function update_densities()
        ...
        if isempty(ni)
            ...
            for i = 1:1:length(ns)
                densities(i) = ns{i}.getProp(PETRandomComparisonBUD.DENSITY);
            end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_denity,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                measure_data = meas.getProp(PETRandomComparisonBUD.RANDOM_COMP_VALUES);
                
                fdr_lim = ones(1,length(ga.getBrainAtlas));
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = measures{value}.getProp(PETRandomComparisonBUD.PVALUE1);
                    p2 = measures{value}.getProp(PETRandomComparisonBUD.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
