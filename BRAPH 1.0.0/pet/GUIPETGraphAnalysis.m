function GUIPETGraphAnalysis(tmp)
%% PET Graph Analysis
% GUIPETGraphAnalysis(ga) opens GUIPETGraphAnalysis (WU, BUT or BUD)
% GUIPETGraphAnalysis(cohort) opens cohort
% GUIPETGraphAnalysis() opens empty analysis with empty cohort

%% General Constants
APPNAME = GUI.PGA_NAME;  % application name
BUILT = BNC.BUILT;
...
%% Application data
if exist('tmp','var') && isa(tmp,'PETGraphAnalysis')
    ga = tmp;
    cohort = ga.getCohort();
    cs = ga.getStructure();
elseif exist('tmp','var') && isa(tmp,'PETCohort')
    cohort = tmp;
...
% Callbacks to manage application data
    function cb_open(~,~)  % (src,event)
        ...
        if filterindex
            ...
            if isa(temp.ga,'PETGraphAnalysis')
                ga = temp.ga;
                if isa(ga,'PETGraphAnalysisWU')
                    GUIPETGraphAnalysisWU(ga)
                elseif isa(ga,'PETGraphAnalysisBUT')
                    GUIPETGraphAnalysisBUT(ga)
                elseif isa(ga,'PETGraphAnalysisBUD')
                    GUIPETGraphAnalysisBUD(ga)
                else
                    cohort = ga.getCohort();
                    cs = ga.getStructure();
                    setup()
    ...
    function cb_import_xml(~,~)  % (scr,event)
        gatmp = update_calc(PETCohort(BrainAtlas()));
...
COHORT_BUTTON_SELECT_TP = 'Select PET cohort.';
COHORT_BUTTON_SELECT_MSG = 'Select file (*.pc) from where to load brain atlas';
...
init_cohort()
    ...
    function update_cohort()
        ...
        if cohort.length>0
            set(ui_text_cohort_name,'String',cohort.getProp(PETCohort.NAME))
    ...
    function cb_cohort(src,~)  % (src,event)
        if strcmp(get(src,'String'),COHORT_BUTTON_VIEW_CMD)
            ...
            try
                ...
                errordlg('The file is not a valid PET Cohort file. Please load a valid .mc file');
            end
        end
    end
...
init_calc()
    ...
    function update_calc_ganame()
        ...
        if isempty(ganame)
            set(f,'Name',[GUI.PGA_NAME ' - ' BNC.VERSION])
        else
            set(f,'Name',[GUI.PGA_NAME ' - ' BNC.VERSION ' - ' ganame])
        end
    ...
    function update_calc()
        ...
        switch graph
            case 1  % weighted undirected
                ga = PETGraphAnalysisWU(cohort,cs, ...
                    PETGraphAnalysis.CORR, PETGraphAnalysis.CORR_OPTIONS{corr}, ...
                    PETGraphAnalysis.NEG, PETGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
            case 2  % binary undirected (fix threshold)
                ga = PETGraphAnalysisBUT(cohort,cs, ...
                    PETGraphAnalysis.CORR, PETGraphAnalysis.CORR_OPTIONS{corr}, ...
                    PETGraphAnalysis.NEG, PETGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
            otherwise  % binary undirected (fix density)
                ga = PETGraphAnalysisBUD(cohort,cs, ...
                    PETGraphAnalysis.CORR, PETGraphAnalysis.CORR_OPTIONS{corr}, ...
                    PETGraphAnalysis.NEG, PETGraphAnalysis.NEG_OPTIONS{neg} ...
                    );
        end
    ...
    function cb_comm_edit(src,~)  % (src,event)
        if ~isempty(cohort.getProps(PETSubject.DATA))
            ...
            fig_structure = GUIPETGraphAnalysisStructure(fig_structure,ga);
    ...
    function cb_calc_set(~,~)  % (src,event)
        if ga.getCohort().groupnumber()>0
            if isa(ga,'PETGraphAnalysisWU')
                GUIPETGraphAnalysisWU(copy(ga))
            elseif isa(ga,'PETGraphAnalysisBUT')
                GUIPETGraphAnalysisBUT(copy(ga))
            elseif isa(ga,'PETGraphAnalysisBUD')
                GUIPETGraphAnalysisBUD(copy(ga))
            end
        end
    end
    function cb_calc_sub(~,~)  % (src,event)
        if ~isempty(cohort.getProps(PETSubject.DATA))
            ...
            fig_structure = GUIPETGraphAnalysisSubgraph(fig_structure,ga);
            waitfor(fig_structure);
