function GUIPETGraphAnalysisBUTSetGraph(f,ga,bg)

APPNAME = [GUI.PGA_NAME];  % application name
NAME_GRAPH = [APPNAME ' : Binary Undirected Graph ' ];
...
set(fig_graph,'Visible','on');
    ...
    function cb_checkbox_linecolor(~,~)  % (src,event)
        if get(ui_checkbox_graph_linecolor,'Value')
            ...
            group = get(ui_popup_graph_group,'Value');
            graph = GraphWU(ga.getA(group));
            A = graph.A;
    ...
    function update_graph()
        ...
        if get(ui_checkbox_graph_weighted,'Value')
            ...
            if get(ui_checkbox_graph_density,'Value')
                ...
                graph = GraphBU(ga.getA(group),'density',density);
            end
            if get(ui_checkbox_graph_threshold,'Value')
                threshold = str2num(get(ui_edit_graph_bt,'String'));
                graph = GraphBU(ga.getA(group),'threshold',threshold);
            end
