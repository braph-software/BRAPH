function GUIPETGraphAnalysisBUD(ga)
%% PET Graph Analysis
% GUIPETGraphAnalysis(ga,true) opens analysis with only reading and basic writing permissions
% GUIPETGraphAnalysis(ga) opens cohort - equivalent to GUIPETGraphAnalysis(ga,false)
% GUIPETGraphAnalysis(cohort) opens empty analysis (the PETCohort is set to cohort)
% GUIPETGraphAnalysis() opens empty analysis with empty cohort

%% General Constants
APPNAME = [GUI.PGA_NAME ' BUD'];  % application name
BUILT = BNC.BUILT;
...
OPEN_SC = GUI.OPEN_SC;
OPEN_TP = ['Open PET graph analysis. Shortcut: ' GUI.ACCELERATOR '+' OPEN_SC];

SAVE_CMD = GUI.SAVE_CMD;
SAVE_SC = GUI.SAVE_SC;
SAVE_TP = ['Save current PET graph analysis. Shortcut: ' GUI.ACCELERATOR '+' SAVE_SC];

SAVEAS_CMD = GUI.SAVEAS_CMD;
SAVEAS_TP = 'Open dialog box to save current PET graph analysis';

IMPORT_XML_CMD = GUI.IMPORT_XML_CMD;
IMPORT_XML_TP = 'Import PET graph analysis from an xml file (*.xml)';

EXPORT_XML_CMD = GUI.EXPORT_XML_CMD;
EXPORT_XML_TP = 'Export PET graph analysis as an xml file (*.xml)';
...
%% Application data
Check.isa('Error: The PET graph analysis ga must be a PETGraphAnalysisBUD',ga,'PETGraphAnalysisBUD')
selected_calc = [];
...
% Callbacks to manage application data
    function cb_open(~,~)  % (src,event)
        % select file
        [file,path,filterindex] = uigetfile(GUI.PGA_EXTENSION,GUI.PGA_MSG_GETFILE);
        % load file
        if filterindex
            filename = fullfile(path,file);
            ga = load(filename,'-mat','ga','selected_calc','selected_brainmeasures','selected_regionmeasures','BUILT');
            if isa(ga.ga,'PETGraphAnalysis')
    ...
    function cb_saveas(~,~)  % (src,event)
        % select file
        [file,path,filterindex] = uiputfile(GUI.PGA_EXTENSION,GUI.PGA_MSG_PUTFILE);
        % save file
        if filterindex
            filename = fullfile(path,file);
            save(filename,'ga','selected_calc','selected_brainmeasures','selected_regionmeasures','BUILT');
    ...
    function cb_import_xml(~,~)  % (scr,event)
        gatmp = PETGraphAnalysisBUD(PETCohort(BrainAtlas()),Structure());
...
COHORT_BUTTON_VIEW_TP = ['Open PET cohort with ' GUI.PCE_NAME '.'];
...
init_cohort()
    ...
    function update_cohort()
        ...
        set(ui_text_cohort_name,'String',ga.getCohort().getProp(PETCohort.NAME))
    ...
    function cb_cohort(~,~)  % (src,event)
        GUIPETCohort(ga.getCohort(),true)  % open atlas with restricted permissions
    end
...
setup_calc_corr()
    function init_calc()
        ...
        set(ui_popup_calc_graph,'String',PETGraphAnalysis.GRAPH_OPTIONS)
        ...
        set(ui_popup_calc_corr,'String',PETGraphAnalysis.CORR_OPTIONS)
        ...
        set(ui_popup_calc_neg,'String',PETGraphAnalysis.NEG_OPTIONS)
    ...
    function update_calc_ganame()
        ganame = ga.getProp(PETCohort.NAME);
        if isempty(ganame)
    ...
    function cb_calc_ganame(~,~)  % (src,event)
        ga.setProp(PETGraphAnalysis.NAME,get(ui_edit_calc_ganame,'String'));
        update_calc_ganame()
    end
    function cb_calc_new(~,~)  %  (src,event)
        GUIPETGraphAnalysis(copy(ga.getCohort()))
    end
    function cb_comm_view(~,~)  % (src,event)
        GUIPETGraphAnalysisStructure(fig_structure,ga,true);
    end
    ...
    function cb_calc_calculate(~,~)  %  (src,event)
        mlist = measurelist();
        GUIPETGraphAnalysisBUDCalcMeas(ga,mlist,selected_calc)
    end
    function cb_calc_compare(~,~)  %  (src,event)
        mlist = measurelist();
        GUIPETGraphAnalysisBUDCalcComp(ga,mlist,selected_calc)
    end
    function cb_calc_random(~,~)  %  (src,event)
        mlist = measurelist();
        GUIPETGraphAnalysisBUDCalcRand(ga,mlist,selected_calc)
    end
    function setup_calc_corr()
        set(ui_popup_calc_corr,'Value',find(strcmp(PETGraphAnalysis.CORR_OPTIONS,ga.getProp(PETGraphAnalysis.CORR))))
        set(ui_popup_calc_graph,'Value',find(strcmp(PETGraphAnalysis.GRAPH_OPTIONS,ga.getProp(PETGraphAnalysis.GRAPH))))
        set(ui_popup_calc_neg,'Value',find(strcmp(PETGraphAnalysis.NEG_OPTIONS,ga.getProp(PETGraphAnalysis.NEG))))
...
init_matrix()
    ...
    function S = update_rearrange()
        ...
        graph = GraphBU(ga.getA(group),'structure',ga.getStructure(),'density',str2num(get(ui_edit_matrix_bs,'String')));
        [S,~] = graph.structure();
    end
    ...
    function update_brainmeasures_table()
        ...
        if get(ui_checkbox_brainmeasures_meas,'Value')
            ...
            for i = 1:1:length(ms)
                ...
                data{i,BRAINMEASURES_MEAS_VALUE_COL} = ms{i}.getProp(PETMeasureBUD.VALUES1);
                data{i,BRAINMEASURES_MEAS_THRESHOLD_COL} = ms{i}.getProp(PETMeasureBUD.THRESHOLD1);
                data{i,BRAINMEASURES_MEAS_DENSITY_COL} = ms{i}.getProp(PETMeasureBUD.DENSITY);
                data{i,BRAINMEASURES_MEAS_NOTES_COL} = ms{i}.getProp(PETMeasureBUD.NOTES);
                data{i,BRAINMEASURES_MEAS_MEASURE_COL} = Graph.NAME{ms{i}.getProp(PETMeasureBUD.CODE)};
                data{i,BRAINMEASURES_MEAS_PARAM_COL} = ms{i}.getProp(PETMeasureBUD.PARAM);
                data{i,BRAINMEASURES_MEAS_GROUP_COL} = ms{i}.getProp(PETMeasureBUD.GROUP1);
            end
        ...
        elseif get(ui_checkbox_brainmeasures_comp,'Value')
            ...
            for i = 1:1:length(cs)
                ...
                data{i,BRAINMEASURES_COMP_PVALUE1_COL} = cs{i}.getProp(PETComparisonBUD.PVALUE1);
                data{i,BRAINMEASURES_COMP_PVALUE2_COL} = cs{i}.getProp(PETComparisonBUD.PVALUE2);
                data{i,BRAINMEASURES_COMP_VALUE1_COL} = cs{i}.getProp(PETComparisonBUD.VALUES1);
                data{i,BRAINMEASURES_COMP_VALUE2_COL} = cs{i}.getProp(PETComparisonBUD.VALUES2);
                
                confint = cs{i}.CI(5);
                data{i,BRAINMEASURES_COMP_CIL_COL} = confint(1);
                data{i,BRAINMEASURES_COMP_CIU_COL} = confint(2);
                
                density = cs{i}.getProp(PETComparisonBUD.DENSITY);
                data{i,BRAINMEASURES_COMP_DENSITY_COL} = density;
                
                threshold1 = cs{i}.getProp(PETComparisonBUD.THRESHOLD1);
                threshold2 = cs{i}.getProp(PETComparisonBUD.THRESHOLD2);
                data{i,BRAINMEASURES_COMP_THRESHOLD1_COL} = threshold1;
                data{i,BRAINMEASURES_COMP_THRESHOLD2_COL} = threshold2;
                
                data{i,BRAINMEASURES_COMP_NOTES_COL} = cs{i}.getProp(PETComparisonBUD.NOTES);
                data{i,BRAINMEASURES_COMP_MEASURE_COL} = Graph.NAME{cs{i}.getProp(PETComparisonBUD.CODE)};
                data{i,BRAINMEASURES_COMP_PARAM_COL} = cs{i}.getProp(PETComparisonBUD.PARAM);
                data{i,BRAINMEASURES_COMP_GROUP1_COL} = cs{i}.getProp(PETComparisonBUD.GROUP1);
                data{i,BRAINMEASURES_COMP_GROUP2_COL} = cs{i}.getProp(PETComparisonBUD.GROUP2);
            end
            set(ui_table_brainmeasures,'Data',data)
        ...
        elseif get(ui_checkbox_brainmeasures_rand,'Value')
            ...
            for i = 1:1:length(ns)
                ...
                data{i,BRAINMEASURES_RAND_COMPVALUE_COL} = ns{i}.getProp(PETRandomComparisonBUD.RANDOM_COMP_VALUES);
                data{i,BRAINMEASURES_RAND_PVALUE1_COL} = ns{i}.getProp(PETRandomComparisonBUD.PVALUE1);
                data{i,BRAINMEASURES_RAND_PVALUE2_COL} = ns{i}.getProp(PETRandomComparisonBUD.PVALUE2);
                data{i,BRAINMEASURES_RAND_REALVALUE_COL} = ns{i}.getProp(PETRandomComparisonBUD.VALUES1);
                
                confint = ns{i}.CI(5);
                data{i,BRAINMEASURES_RAND_CIL_COL} = confint(1);
                data{i,BRAINMEASURES_RAND_CIU_COL} = confint(2);
                
                density = ns{i}.getProp(PETRandomComparisonBUD.DENSITY);
                data{i,BRAINMEASURES_RAND_DENSITY_COL} = density;
                
                threshold = ns{i}.getProp(PETRandomComparisonBUD.THRESHOLD1);
                data{i,BRAINMEASURES_RAND_THRESHOLD_COL} = threshold;
                
                data{i,BRAINMEASURES_RAND_NOTES_COL} = ns{i}.getProp(PETRandomComparisonBUD.NOTES);
                data{i,BRAINMEASURES_RAND_MEASURE_COL} = Graph.NAME{ns{i}.getProp(PETRandomComparisonBUD.CODE)};
                data{i,BRAINMEASURES_RAND_PARAM_COL} = ns{i}.getProp(PETRandomComparisonBUD.PARAM);
                data{i,BRAINMEASURES_RAND_GROUP_COL} = ns{i}.getProp(PETRandomComparisonBUD.GROUP1);
            end
                ...
                if get(ui_checkbox_brainmeasures_meas,'Value')
                    ...
                    if ~isempty(mi)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ms)
                                meas(i) = ms{i}.getProp(PETMeasureBUD.VALUES1);
                                d(i) = ms{i}.getProp(PETMeasureBUD.DENSITY);
                            end
                ...
                elseif get(ui_checkbox_brainmeasures_comp,'Value')
                    ...
                    if ~isempty(ci)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(cs)
                                diff(i) = cs{i}.diff();
                                d(i) = cs{i}.getProp(PETMeasureBUD.DENSITY);
                                m(i) = cs{i}.CI(50);
                                ci5 = cs{i}.CI(5);
                                ci5l(i) = ci5(1);
                                ci5u(i) = ci5(2);
                ...
                elseif get(ui_checkbox_brainmeasures_rand,'Value')
                    ...
                    if ~isempty(ni)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ns)
                                meas(i) = ns{i}.getProp(PETRandomComparisonBUD.RANDOM_COMP_VALUES);
                                d(i) = ns{i}.getProp(PETRandomComparisonBUD.DENSITY);
                                m(i) = ns{i}.CI(50);
                                ci5 = ns{i}.CI(5);
                                ci5l(i) = ci5(1);
                                ci5u(i) = ci5(2);
    ...
    function update_regionmeasures_table()
        ...
        if get(ui_checkbox_regionmeasures_meas,'Value')
            ...
            for i = 1:1:length(ms)
                ...
                data1 = ms{i}.getProp(PETMeasureBUD.VALUES1);
                data{i,REGIONMEASURES_MEAS_VALUE_COL} = data1(bri);
                
                data{i,REGIONMEASURES_MEAS_THRESHOLD_COL} = ms{i}.getProp(PETMeasureBUD.THRESHOLD1);
                data{i,REGIONMEASURES_MEAS_DENSITY_COL} = ms{i}.getProp(PETMeasureBUD.DENSITY);
                data{i,REGIONMEASURES_MEAS_NOTES_COL} = ms{i}.getProp(PETMeasureBUD.NOTES);
                data{i,REGIONMEASURES_MEAS_MEASURE_COL} = Graph.NAME{ms{i}.getProp(PETMeasureBUD.CODE)};
                data{i,REGIONMEASURES_MEAS_GROUP_COL} = ms{i}.getProp(PETMeasureBUD.GROUP1);
                data{i,REGIONMEASURES_MEAS_BR_COL} = BrainRegions{bri};
                data{i,REGIONMEASURES_MEAS_PARAM_COL} = ms{i}.getProp(PETMeasureBUD.PARAM);
            end
        ...
        elseif get(ui_checkbox_regionmeasures_comp,'Value')
            ...
            set(ui_table_regionmeasures,'ColumnName',{'',' difference ',' p (1-tailed) ',' fdr (1-tailed) ',' p (2-tailed) ',' fdr (2-tailed) ',' value 1 ',' value 2 ',' CI lower ',' CI upper ',' density ',' threshold 1 ',' threshold 2 ',' notes ',' measure ',' param ',' group 1 ',' group 2 ',' brainregion '})
            ...
            for i = 1:1:length(cs)
                ...
                threshold1 = cs{i}.getProp(PETComparisonBUD.THRESHOLD1);
                threshold2 = cs{i}.getProp(PETComparisonBUD.THRESHOLD2);
                data{i,REGIONMEASURES_COMP_THRESHOLD1_COL} = threshold1;
                data{i,REGIONMEASURES_COMP_THRESHOLD2_COL} = threshold2;
                
                data{i,REGIONMEASURES_COMP_NOTES_COL} = cs{i}.getProp(PETComparisonBUD.NOTES);
                data{i,REGIONMEASURES_COMP_MEASURE_COL} = Graph.NAME{cs{i}.getProp(PETComparisonBUD.CODE)};
                data{i,REGIONMEASURES_COMP_PARAM_COL} = cs{i}.getProp(PETComparisonBUD.PARAM);
                data{i,REGIONMEASURES_COMP_GROUP1_COL} = cs{i}.getProp(PETComparisonBUD.GROUP1);
                data{i,REGIONMEASURES_COMP_GROUP2_COL} = cs{i}.getProp(PETComparisonBUD.GROUP2);
            ...
            for i = 1:1:length(ns)
                ...
                data{i,REGIONMEASURES_RAND_NOTES_COL} = ns{i}.getProp(PETRandomComparisonBUD.NOTES);
                data{i,REGIONMEASURES_RAND_MEASURE_COL} = Graph.NAME{ns{i}.getProp(PETRandomComparisonBUD.CODE)};
                data{i,REGIONMEASURES_RAND_PARAM_COL} = ns{i}.getProp(PETRandomComparisonBUD.PARAM);
                data{i,REGIONMEASURES_RAND_GROUP_COL} = ns{i}.getProp(PETRandomComparisonBUD.GROUP1);
                ...
                if get(ui_checkbox_regionmeasures_meas,'Value')
                    ...
                    if ~isempty(mi)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ms)
                                meas1 = ms{i}.getProp(PETMeasureBUD.VALUES1);
                                meas(i) = meas1(bri);
                                d(i) = ms{i}.getProp(PETMeasureBUD.DENSITY);
                            end
                    ...
                    if ~isempty(ni)
                        ...
                        if isempty(strmatch(name,str2))
                            ...
                            for i = 1:1:length(ns)
                                ...
                                temp = ns{i}.getProp(PETRandomComparisonBUD.RANDOM_COMP_VALUES);
                                meas(i) = temp(bri);
...
init_brainview()
    ...
    function cb_brainview_viewgroup(~,~)  % (src,event)
        ...
        if ga.getCohort().length()>0
            GUIPETGraphAnalysisBUDViewMeas(fig_group,ga,bg)
        else
    ...
    function cb_brainview_viewcomparison(~,~)  % (src,event)
        ...
        if ga.getCohort().length()>0
            GUIPETGraphAnalysisBUDViewComp(fig_group,ga,bg)
        else
    ...
    function cb_brainview_viewrandom(~,~)  % (src,event)
        ...
        if ga.getCohort().length()>0
            GUIPETGraphAnalysisBUDViewRand(fig_group,ga,bg)
        else
    ...
    function cb_brainview_graph(~,~)  % (src,event)
        ...
        GUIPETGraphAnalysisBUDSetGraph(f,ga,bg)
    end
    ...
    function cb_menu_figure(~,~)  % (src,event)
        ...
        elseif strcmpi(get(ui_panel_brainview,'Visible'),'on')
            ...
            elseif strcmp(BRAINVIEW_BUTTON_STATE,BRAINVIEW_BUTTON_VIEWMEAS)
                h = figure('Name', BRAINVIEW_BUTTON_VIEWMEAS);%[BRAINVIEW_BUTTON_VIEWMEAS ' - ' ga.getCohort().getGroup(get(ui_popup_brainview_comp_group1,'Value')).getProp(Group.NAME)]);
