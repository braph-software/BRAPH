classdef PETGraphAnalysis < List
    % PETGraphAnalysis < List (Abstract) : Graph analysis of PET data
    %   PETGraphAnalysis represents a list of measures used for graph analysis of PET data.
    %   Instances of this class cannot be created. Use one of the subclasses 
    %   (e.g., PETGraphAnalysisBUD).
    %
    % PETGraphAnalysis properties (Access = protected):
    %   props           -   cell array of object properties < ListElement
    %   elements        -   cell array of list elements < List
    %   path            -   XML file path < List
    %   file            -   XML file name < List
    %   cohort          -   cohort (PETCohort)
    ...
    %
    % PETGraphAnalysis properties (Constant):
    ...
    %
    % PETGraphAnalysis methods (Access = protected):
    %   PETGraphAnalysis        -   constructor
    %   initialize              -   initializes graph analysis
    %   copyElement             -   deep copy
    %
    % PETGraphAnalysis methods (Abstract,Access = protected):
    %   initialize_hashtables   -   initialize hash tables
    % 
    % PETGraphAnalysis methods (Abstract):
    ...
    %
    % PETGraphAnalysis methods (Abstract, Static):
    ...
    %
    % PETGraphAnalysis methods (Static):
    ...
    %
    % See also PETGraphAnalysisBUT, PETGraphAnalysisBUD ,PETGraphAnalysisWU, List.
    ...
    properties (Constant)
        ...
        NAME_DEFAULT = 'PET Graph Analysis'
        ...
        GRAPH_BUT = 'but'
        GRAPH_BUD = 'bud'
        GRAPH_OPTIONS = {PETGraphAnalysis.GRAPH_WU ...
            PETGraphAnalysis.GRAPH_BUT ...
            PETGraphAnalysis.GRAPH_BUD}
        GRAPH_DEFAULT = PETGraphAnalysis.GRAPH_BUD
        ...
        CORR_OPTIONS = {PETGraphAnalysis.CORR_PEARSON ...
        ...
        CORR_DEFAULT = PETGraphAnalysis.CORR_PEARSON
        ...
        NEG_NONE = 'none'
        NEG_ABS = 'abs'
        NEG_OPTIONS = {PETGraphAnalysis.NEG_ZERO ...
            PETGraphAnalysis.NEG_NONE ....
            PETGraphAnalysis.NEG_ABS}
        NEG_DEFAULT = PETGraphAnalysis.NEG_NONE
    ...
    methods (Access = protected)
        function ga = PETGraphAnalysis(cohort,structure,varargin)
            % PETGRAPHANALYSIS(COHORT,STRUCTURE) creates a PET graph analysis with default
            %   properties using the cohort COHORT and community structure STRUCTURE.
            %   This method is only accessible by the subclasses of PETGraphAnalysis.
            %
            % PETGRAPHANALYSIS(COHORT,STRUCTURE,Tag1,Value1,Tag2,Value2,...) initializes property Tag1 to Value1, 
            %   Tag2 to Value2, ... .
            %   Admissible properties are:
            %     PETGraphAnalysis.NAME      -   char
            %     PETGraphAnalysis.GRAPH     -   options (PETGraphAnalysis.GRAPH_WU,
            %                                    PETGraphAnalysis.GRAPH_BUT,PETGraphAnalysis.GRAPH_BUD)
            %     PETGraphAnalysis.CORR      -   options (PETGraphAnalysis.CORR_PEARSON,PETGraphAnalysis.CORR_SPEARMAN,
            %                                    PETGraphAnalysis.CORR_KENDALL,PETGraphAnalysis.CORR_PARTIALPEARSON,
            %                                    PETGraphAnalysis.CORR_PARTIALSPEARMAN)
            %     PETGraphAnalysis.NEG       -   options (PETGraphAnalysis.NEG_ZERO,
            %                                    PETGraphAnalysis.NEG_NONE,PETGraphAnalysis.NEG_ABS)
            %
            % See also PETGraphAnalysis,  List.
            
            Check.isa('The variable cohort must be a PETCohort',cohort,'PETCohort')
        ...
        function cp = copyElement(ga)
            ...
            %
            % See also PETGraphAnalysis, handle, matlab.mixin.Copyable.
    ...
    methods
        function [A,P] = adjmatrix(ga,data)
            ...
            %
            % See also PETGraphAnalysis, corr, partialcorr, corrcoef.
            
            switch ga.getProp(PETGraphAnalysis.CORR)
                case PETGraphAnalysis.CORR_SPEARMAN
                    [A,P] = corr(data,'Type','Spearman');
                case PETGraphAnalysis.CORR_KENDALL
                    [A,P] = corr(data,'Type','Kendall');
                case PETGraphAnalysis.CORR_PARTIALPEARSON
                    [A,P] = partialcorr(data,'Type','Pearson');
                case PETGraphAnalysis.CORR_PARTIALSPEARMAN
                    [A,P] = partialcorr(data,'Type','Spearman');
                otherwise  % PETGraphAnalysis.PEARSON
                    [A,P] = corrcoef(data);                
            end
            
            switch ga.getProp(PETGraphAnalysis.NEG)
                case PETGraphAnalysis.NEG_ZERO
                    A(A<0) = 0;
                case PETGraphAnalysis.NEG_ABS
                    A = abs(A);
                otherwise  % PETGraphAnalysis.NEG_NONE
            end
        end        
        function disp(ga)
            % DISP displays graph analysis
        ...
        function cohort = getCohort(ga)
            ...
            %
            % See also PETGraphAnalysis, PETCohort.
        ...
        function atlas = getBrainAtlas(ga)
            ...
            %
            % See also PETGraphAnalysis, PETCohort, BrainAtlas.
        ...
        function bs = getPlotBrainSurf(ga)
            ...
            %
            % See also PETGraphAnalysis, PETCohort, PlotBrainSurf.
        ...
        function ba = getPlotBrainAtlas(ga)
            ...
            %
            % See also PETGraphAnalysis, PETCohort, PlotBrainAtlas.
        ...
        function bg = getPlotBrainGraph(ga)
            ...
            %
            % See also PETGraphAnalysis, PlotBrainGraph.
        ...
        function [bool,i] = existMeasure(ga,m)
            ...
            index = ga.getIndex(m);
            i = ga.ht_measure{m.getProp(PETMeasure.CODE)}(index);
            bool = i>0;
        end
        function [bool,i] = existComparison(ga,c)
            ...
            index = ga.getIndex(c);
            i = ga.ht_comparison{c.getProp(PETMeasure.CODE)}(index(1),index(2));
            bool = i>0;
        end
        function [bool,i] = existRandom(ga,n)
            ...
            index = ga.getIndex(n);
            i = ga.ht_random_comparison{n.getProp(PETMeasure.CODE)}(index);
            bool = i>0;
        end
        function add(ga,m,i)
            ...
            if m.isMeasure()
                index = ga.getIndex(m);
                ga.ht_measure{m.getProp(PETMeasure.CODE)}(index) = i;
            elseif m.isComparison()
                index = ga.getIndex(m);
                ga.ht_comparison{m.getProp(PETMeasure.CODE)}(index(1),index(2)) = i;
            else % m.isRandom()
                index = ga.getIndex(m);
                ga.ht_random_comparison{m.getProp(PETMeasure.CODE)}(index) = i;
            end
        end
        function remove(ga,i)
            ...
            if i>0 && i<=ga.length()
                ...
                if m.isMeasure()
                    index = ga.getIndex(m);
                    ga.ht_measure{m.getProp(PETMeasure.CODE)}(index) = 0;
                elseif m.isComparison()
                    index = ga.getIndex(m);
                    ga.ht_comparison{m.getProp(PETMeasure.CODE)}(index(1),index(2)) = 0;
                else % m.isRandom()
                    index = ga.getIndex(m);
                    ga.ht_random_comparison{m.getProp(PETMeasure.CODE)}(index) = 0;
                end
        ...
        function [ListNode,Document,RootNode] = toXML(ga,Document,RootNode)
            ...
            %
            % See also PETGraphAnalysis, PETCohort, List, xmlwrite.
        ...
        function fromXML(ga,ListNode)
            % FROMXML loads PETGraphAnalysis from XML Node
            %
            % FROMXML(GA,LISTNODE) loads the properties of PETGraphAnalysis GA,
            %   from the attributes of LISTNODE.
            %   
            % See also PETGraphAnalysis, PETCohort, List, xmlread.
    ...
    methods (Static)
        function N = propnumber()
            % PROPNUMBER number of properties
            %
            % N = PROPNUMBER() gets the total number of properties of PETGraphAnalysis.
            %
            % See also PETGraphAnalysis.
        ...
        function tags = getTags()
            ...
            %   of all properties of PETGraphAnalysis.
            %
            % See also PETGraphAnalysis.
            
            tags = {PETGraphAnalysis.NAME_TAG ...
                PETGraphAnalysis.GRAPH_TAG ...
                PETGraphAnalysis.CORR_TAG ...
                PETGraphAnalysis.NEG_TAG};
        end
        function formats = getFormats()
            ...
            formats = {PETGraphAnalysis.NAME_FORMAT ...
                PETGraphAnalysis.GRAPH_FORMAT ...
                PETGraphAnalysis.CORR_FORMAT ...
                PETGraphAnalysis.NEG_FORMAT};
        end
        function defaults = getDefaults()
            ...
            defaults = {PETGraphAnalysis.NAME_DEFAULT ...
                PETGraphAnalysis.GRAPH_DEFAULT ...
                PETGraphAnalysis.CORR_DEFAULT ...
                PETGraphAnalysis.NEG_DEFAULT};
        end
        function options = getOptions(prop)  % (prop)
            % GETOPTIONS cell array with options (only for properties with options format)
            %
            % OPTIONS = GETOPTIONS(PROP) returns a cell array containing the options
            %   of the properties specified by PROP of PETGraphAnalysis.
            %   If the PROP is not "options", it returns an empty cell array. 
            %
            % See also PETGraphAnalysis.
            
            switch prop
                case PETGraphAnalysis.GRAPH
                    options = PETGraphAnalysis.GRAPH_OPTIONS;
                case PETGraphAnalysis.CORR
                    options = PETGraphAnalysis.CORR_OPTIONS;
                case PETGraphAnalysis.NEG
                    options = PETGraphAnalysis.NEG_OPTIONS;
                otherwise
                    options = {};
            end
        end        
