function GUIPETCohort(tmp,restricted)
%% PET Cohort Editor
...
%% General Constants
APPNAME = GUI.PCE_NAME;  % application name
BUILT = BNC.BUILT;
...
OPEN_SC = GUI.OPEN_SC;
OPEN_TP = ['Open PET cohort. Shortcut: ' GUI.ACCELERATOR '+' OPEN_SC];

SAVE_CMD = GUI.SAVE_CMD;
SAVE_SC = GUI.SAVE_SC;
SAVE_TP = ['Save current PET cohort. Shortcut: ' GUI.ACCELERATOR '+' SAVE_SC];

SAVEAS_CMD = GUI.SAVEAS_CMD;
SAVEAS_TP = 'Open dialog box to save current PET cohort';

IMPORT_XML_CMD = GUI.IMPORT_XML_CMD;
IMPORT_XML_TP = 'Import PET cohort from an xml file (*.xml)';

EXPORT_XML_CMD = GUI.EXPORT_XML_CMD;
EXPORT_XML_TP = 'Export PET cohort as an xml file (*.xml)';
...
PETGRAPHANALYSIS_CMD = 'New PET graph analysis ...';
PETGRAPHANALYSIS_TP = ['Generate new PET graph analysis and opens it with ' GUI.MGA_NAME];

%% Application data
if exist('tmp','var') && isa(tmp,'PETCohort')
    cohort = tmp;
elseif exist('tmp','var') && isa(tmp,'BrainAtlas')
    cohort = PETCohort(tmp);
else
    cohort = PETCohort(BrainAtlas());
end
selected_group = [];
selected_subjects = [];
...
% Callbacks to manage application data
    ...
    function cb_saveas(~,~)  % (src,event)
        % select file
        [file,path,filterindex] = uiputfile(GUI.PCE_EXTENSION,GUI.PCE_MSG_PUTFILE);
        % save file
        if filterindex
            filename = fullfile(path,file);
            save(filename,'cohort','selected_group','selected_subjects','BUILT');
    ...
    function cb_import_xml(~,~)  % (scr,event)
        cohorttmp = PETCohort(BrainAtlas());
...
init_atlas()
    ...
    function update_atlas()
        if cohort.getBrainAtlas().length>0
            set(ui_text_atlas_name,'String',cohort.getBrainAtlas().getProp(PETCohort.NAME))
...
GRTAB_LOAD_XML_TP = 'Load subjects from another PET cohort XML file. All subjects will be in the same (new) group';
...
init_grtab()
    ...
    function update_grtab_cohortname()
        cohortname = cohort.getProp(PETCohort.NAME);
        if isempty(cohortname)
            set(f,'Name',[GUI.PCE_NAME ' - ' BNC.VERSION])
        else
            set(f,'Name',[GUI.PCE_NAME ' - ' BNC.VERSION ' - ' cohortname])
        end
    ...
    function cb_grtab_cohortname(~,~)  % (src,event)
        cohort.setProp(PETCohort.NAME,get(ui_edit_grtab_cohortname,'String'));
        update_grtab_cohortname()
    end
    function cb_grtab_load_xls(~,~)  % (src,event)
        try
        cohorttmp = PETCohort(cohort.getBrainAtlas());
        if cohorttmp.loadfromxls(BNC.XLS_MSG_GETFILE)
            ...
            errordlg('The file is not a valid PET Subjects file. Please load a valid Excel file');
        end
    end
    function cb_grtab_load_txt(~,~)  % (src,event)
        ...
        if cohorttmp.loadfromtxt(BNC.TXT_MSG_GETFILE)
            ...
            errordlg('The file is not a valid PET Subjects file. Please load a valid Txt file');
        end
    end
    function cb_grtab_load_xml(~,~)  % (src,event)
        ...
        if cohorttmp.loadfromfile(BNC.XML_MSG_GETFILE)
            ...
            errordlg('The file is not a valid PET Subjects file. Please load a valid Xml file');
        end
    end
...
init_groups()
    ...
    function update_groups(varargin)
        ...
        ColumnFormat = {'logical','char','numeric',PETSubject.GENDER_OPTIONS,'char'};
        for g = 1:1:cohort.groupnumber()
        ...
        for i = 1:1:cohort.length()
            ...
            data{i,TAB_GROUPS_SUBCODE_COL} = cohort.get(i).getProp(PETSubject.CODE);
            data{i,TAB_GROUPS_SUBAGE_COL} = cohort.get(i).getProp(PETSubject.AGE);
            data{i,TAB_GROUPS_SUBGENDER_COL} = cohort.get(i).getProp(PETSubject.GENDER);
            data{i,TAB_GROUPS_SUBNOTES_COL} = cohort.get(i).getProp(PETSubject.NOTES);
        end
        for g = 1:1:cohort.groupnumber()
    ...
    function cb_groups_edit_sub(~,event)  % (src,event)
        ...
        switch col
            ...
            case TAB_GROUPS_SUBCODE_COL
                cohort.get(i).setProp(PETSubject.CODE,newdata)
            case TAB_GROUPS_SUBAGE_COL
                cohort.get(i).setProp(PETSubject.AGE,newdata)
            case TAB_GROUPS_SUBGENDER_COL
                cohort.get(i).setProp(PETSubject.GENDER,newdata)
            case TAB_GROUPS_SUBNOTES_COL
                cohort.get(i).setProp(PETSubject.NOTES,newdata)
            otherwise
                g = col-5;
...
init_subjects()
    ...
    function update_subjects(varargin)
        ...
        for j = 1:1:length(indices)
            if length(cohort.get(indices(j)).getProp(PETSubject.DATA)) ~= cohort.getBrainAtlas.length()
                cohort.get(j).setProp(PETSubject.DATA,zeros(1,cohort.getBrainAtlas.length()))
            end
            RowName{j} = cohort.get(indices(j)).getProp(PETSubject.CODE);
            data(j,:) = [indices(j) cohort.get(indices(j)).getProp(PETSubject.DATA)];
        end
    ...
    function cb_subjects_edit_data(~,event)  % (src,event)
        ...
        switch col
            ...
            otherwise
                ...
                i = indices(j);
                n = col-1;                
                data = cohort.get(i).getProp(PETSubject.DATA);
                data(n) = newdata;
                cohort.get(i).setProp(PETSubject.DATA,data);
        end
        update_grtab_table()
        update_subjects()
    end
...
init_brainview()
    ...
    function cb_brainview_viewsubject(~,~)  %  (src,event)
        ...
        if ~isempty(cohort.getProps(PETSubject.DATA))
            fig_subject = GUIPETCohortViewSubject(fig_subject,cohort,ba);
        else
    ...
    function cb_brainview_viewgroup(~,~)  %  (src,event)
        ...
        if cohort.groupnumber()>0
            fig_group = GUIPETCohortViewGroup(fig_group,cohort,ba);
        else
    ...
    function cb_brainview_viewcomparison(~,~)  %  (src,event)
        ...
        if cohort.groupnumber()>0
            fig_comparison = GUIPETCohortViewComparison(fig_comparison,cohort,ba);
        else
            errordlg('Select a cohort in order to view group settings','Select a cohort','modal');
        end
    end
...
init_menu()
    function init_menu()
        ...
        set(ui_menu_analysis_ga,'Label',PETGRAPHANALYSIS_CMD)
        set(ui_menu_analysis_ga,'Callback',{@cb_menu_ga})
    end
    function cb_menu_figure(~,~)  % (src,event)
        h = figure('Name', ['MCI Cohort - ' cohort.getProp(PETCohort.NAME)]);
        set(gcf,'color','w')
    ...
    function cb_menu_ga(~,~)  % (src,event)
        if cohort.length()>0
            GUIPETGraphAnalysis(cohort.copy())
        else
            msgbox('In order to create an PET analysis the cohort must have at least one subject.', ...
                'Warning: Empty PET cohort', ...
                'warn')
        end
    end
