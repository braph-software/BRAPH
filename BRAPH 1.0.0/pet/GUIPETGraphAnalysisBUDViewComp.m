function GUIPETGraphAnalysisBUDViewComp(fig_group,ga,bg)
...
%% Callback functions
    ...
    function update_densities()
        ...
        if isempty(mi)
            ...
            for i = 1:1:length(ms)
                densities(i) = ms{i}.getProp(PETComparisonBUD.DENSITY);
            end
        ...
        if strcmp(get(ui_list_denity,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = measures{value}.getProp(PETComparisonBUD.PVALUE1);
                    p2 = measures{value}.getProp(PETComparisonBUD.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
