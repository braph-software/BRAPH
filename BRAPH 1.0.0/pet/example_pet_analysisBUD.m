...
%   density created from PET cohort.
%
% See also PETGraphAnalysisBUD, PETCohort.
...
ga = PETGraphAnalysisBUD(cohort,Structure(), ...
    ...
    switch lower(MCF)
        ...
        case 'm'
            ...
            if Graph.isglobal(measurecode)
                ...
                disp(['Measure value = ' num2str(m.getProp(PETMeasureBUD.VALUES1))])
                disp('=== === ===')
            end
            ...
            if Graph.isnodal(measurecode)
                ...
                values = m.getProp(PETMeasureBUD.VALUES1);
        ...
        case 'c'
            ...
            if Graph.isglobal(measurecode)
                ...
                disp(['p-value (1) = ' num2str(c.getProp(PETComparisonBUD.PVALUE1))])
                disp(['p-value (2) = ' num2str(c.getProp(PETComparisonBUD.PVALUE2))])
            ...
            if Graph.isnodal(measurecode)
                ...
                values = c.diff();
                p1 = c.getProp(PETComparisonBUD.PVALUE1);
                p2 = c.getProp(PETComparisonBUD.PVALUE2);
                ci = c.CI(5);
        ...
        case 'r'
            ...
            if Graph.isglobal(measurecode)
                ...
                disp(['Measure code = ' int2str(measurecode) ' GLOBAL MEASURE'])
                disp(['Measure name = ' Graph.NAME{measurecode}])
                disp(['Measure value = ' num2str(mean(n.getProp(PETRandomComparisonBUD.VALUES1)))])
                disp(['Random graph value = ' num2str(mean(n.getProp(PETRandomComparisonBUD.RANDOM_COMP_VALUES)))])
                disp(['p-value (1) = ' num2str(n.getProp(PETRandomComparisonBUD.PVALUE1))])
                disp(['p-value (2) = ' num2str(n.getProp(PETRandomComparisonBUD.PVALUE2))])
            ...
            if Graph.isnodal(measurecode)
                ...
                values = n.getProp(PETRandomComparisonBUD.VALUES1);
                disp(['Average (over regions) measure value = ' num2str(mean(values))])

                values = n.diff();
                p1 = n.getProp(PETRandomComparisonBUD.PVALUE1);
                p2 = n.getProp(PETRandomComparisonBUD.PVALUE2);
                ci = n.CI(5);
                disp(['Differences (per region) = ' num2str(values)])
                disp(['p-value (1) (per region) = ' num2str(p1)])
                disp(['p-value (2) (per region) = ' num2str(p2)])
