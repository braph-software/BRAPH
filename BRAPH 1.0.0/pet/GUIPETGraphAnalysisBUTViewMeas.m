function GUIPETGraphAnalysisBUTViewMeas(fig_group,ga,bg)
...
set(ui_list_threshold,'TooltipString','Select thresholds');
...
%% Callback functions
    ...
    function update_thresholds()
        ...
        if isempty(mi)
            ...
            thresholds = zeros(1,length(ms));
            for i = 1:1:length(ms)
                thresholds(i) = ms{i}.getProp(PETMeasureBUT.THRESHOLD);
            end
    ...
    function update_measure_data()
        ...
        if strcmp(get(ui_list_threshold,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                measure_data = meas.getProp(PETMeasureBUT.VALUES1);
            end
        end
    end
