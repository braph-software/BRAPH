classdef PETCohort < ListWithGroups
    % PETCohort < ListWithGroups : cohort of PET
    %   PETCohort represents a cohort of subjects with PET data.
    %
    % PETCohort properties (Access = protected):
    ...
    %   atlas           -   brain atlas
    %
    % PETCohort properties:
    %   grs             -   group list < ListWithGroups
    %
    % PETCohort properties (Constants):
    ...
    %   element             -   creates new PET subject
    ...
    %   loadfromxls         -   loads PET cohort from XLS file
    %   savetotxt           -   saves a PET cohort to TXT file
    %   loadfromtxt         -   loads PET cohort from TXT file
    %   toXML               -   creates XML Node from PETCohort
    %   fromXML             -   loads PETCohort from XML Node
    %   clear               -   clears PETCohort
    %   copy                -   deep copy < matlab.mixin.Copyable
    %   extract             -   extracts subcommunity cohort of certain subjects
    %
    % PETCohort methods (Access = protected):
    %   copyElement     -   copy cohort and its elements
    %
    % PETCohort methods (Static):
    ...
    %   getOptions      -   PETCohortcell array with options (only for properties with options format)  
    %
    ...
    methods
        function cohort = PETCohort(atlas,subjects,varargin)
            % PETCOHORT(ATLAS,SUBJECTS) creates a PET cohort with default
            %   properties using the atlas ATLAS and subjects SUBJECTS.
            %
            % PETCOHORT(ATLAS,SUBJECTS,Tag1,Value1,Tag2,Value2,...) initializes property Tag1 to Value1, 
            %   Tag2 to Value2, ... .
            %   Admissible properties are:
            %     PETCohort.NAME      -   char
            %
            % See also PETCohort,  ListWithGroups, BrainAtlas.
        ...
        function disp(cohort)
            ...
            for i = 1:1:cohort.length()
                sub = cohort.get(i);
                disp([sub.getPropValue(PETSubject.CODE) ...
                    ' ' sub.getPropValue(PETSubject.AGE) ...
                    ' ' sub.getPropValue(PETSubject.GENDER) ...
                    ' ' sub.getPropValue(PETSubject.DATA) ...
                    ' ' sub.getPropValue(PETSubject.NOTES) ...
                    ])
            end
            disp(' >> GROUPS << ')
        ...
        function sub = element(cohort)
            % ELEMENT creates new empty PET subject
            %
            % SUB = ELEMENT(COHORT) returns an empty PET subject SUB of cohort COHORT.
            %
            % See also PETCohort, PETSubject, BrainAtlas.
        ...
        function bs = getPlotBrainSurf(cohort)
            ...
            %
            % See also PETCohort, PlotBrainSurf.
        ...
        function ba = getPlotBrainAtlas(cohort)
            ...
            %
            % See also PETCohort, PlotBrainAtlas.
        ...
        function data = getSubjectData(cohort,g)
            ...
            for j = 1:1:length(indices)
                data(j,:) = cohort.get(indices(j)).getProp(PETSubject.DATA);
            end
        end
        ...
        function [dm,p_single,p_double,m1,m2,s1,s2,all1,all2,dall] = comparison(cohort,g1,g2,M,varargin)
            ...
            %                      'false' (default) | 'true'
            %
            % See also PETCohort, pvalue1, pvalue2.
        ...
        function success = loadfromxls(cohort,msg)
            ...
            %   PET cohort by loading an XLS file ('*.xlsx' or '*.xls').
            %   It returns true if the PET cohort is successfully created.
            %
            % See also PETCohort, uigetfile, fopen.
            ...
            if filterindex
                ...
                for i = 2:1:size(raw,1)
                    ...
                    sub = PETSubject( ...
                        PETSubject.CODE,raw{i,1}, ...
                        PETSubject.DATA,cell2mat(raw(i,2:size(raw,2))));
                    cohort.add(sub)
                end
        ...
        function savetotxt(cohort,msg)
            % SAVETOTXT saves a PET cohort to TXT file 
            %
            % SAVETOTXT(COHORT,MSG) saves PETCohort COHORT to a txt file.
            %   Uses a dialog box to select the TXT file ('*.txt').
            %
            % See also PETCohort, uiputfile, fopne, fprintf.
            ...
            if filterindex
                ...
                if fid~=-1  % could not open the file
                    ...
                    for j = 1:1:cohort.length()
                        sub = cohort.get(j);
                        fprintf(fid,'%s\t',sub.getPropValue(PETSubject.CODE));
                        data = sub.getProp(PETSubject.DATA);
        ...
        function success = loadfromtxt(cohort,msg)
            ...
            %   PET cohort by loading a TXT file ('*.txt)'.
            %   It returns true if the PET cohort is successfully created.
            %
            % See also PETCohort, uigetfile, fopen.
            ...
            if filterindex
                ...
                if fid~=-1  % could not open the file
                    ...
                    for i = 1:1:length(indices1)
                        ...
                        sub = PETSubject( ...
                            PETSubject.CODE,txti(1:indices2(1)-1), ...
                            PETSubject.DATA,txti(indices2(1):end-1));
                        cohort.add(sub)
                    end
        ...
        function [ListNode,Document,RootNode] = toXML(cohort,Document,RootNode)
            ...
            %
            % See also PETCohort, ListWithGroups, List, xmlwrite.
        ...
        function fromXML(cohort,ListNode)
            % FROMXML loads PETCohort from XML Node
            %
            % FROMXML(COHORT,LISTNODE) loads the properties of PETCohort COHORT,
            %   from the attributes of LISTNODE.
            %   
            % See also PETCohort, ListWithGroups, List, xmlread.
        ...
        function clear(cohort) % cohort.atlas.clear();
            ...
            %   PETCohort COHORT to their default values and eliminates 
            %   all list elements.
            %
            % See also PETCohort, ListWithGroups, List.
        ...
        function scohort = extract(cohort,brs,subs)
            ...
            for j = 1:1:scohort.length()
                data = scohort.get(j).getProp(PETSubject.DATA);
                scohort.get(j).setProp(PETSubject.DATA,data(brs));
            end
        end
    end
    methods (Access = protected)
        function cp = copyElement(cohort)
            ...
            %
            % See also PETCohort, handle, matlab.mixin.Copyable.
    ...
    methods (Static)
        function N = propnumber()
            % PROPNUMBER number of properties
            %
            % N = PROPNUMBER() gets the total number of properties of PETCohort.
            %
            % See also PETCohort.
        ...
        function tags = getTags()
            ...
            %   of all properties of PETCohort.
            %
            % See also PETCohort, ListElement.
            
            tags = {PETCohort.NAME_TAG};
        end
        ...
        function defaults = getDefaults()
            ...
            defaults = {PETCohort.NAME_DEFAULT};
        end
        function options = getOptions(~)  % (prop)
            ...
            %   of the properties specified by PROP of PETCohort.
