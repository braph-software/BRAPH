function GUIPETGraphAnalysisBUTCalcMeas(ga,mlist,selected_calc)
...
%% General Constants
APPNAME = [GUI.PGA_NAME 'fixed threshold'];  % application name
    ...
    function calculate(ga,mlist,g,threshold)
        ...
        for vi = vi:1:length(threshold)
            for mi = 1:1:length(mlist)
                ...
                msg = ['time = ' int2str(toc(start)) '.' int2str(mod(toc(start)*10,10)) 's - density = ' num2str(m.getProp(PETMeasureBUT.THRESHOLD)) ' (' num2str(threshold(vi)) ') - group = ' int2str(g) ' - ' Graph.NAME{m.getProp(PETMeasure.CODE)}];
