function GUIPETGraphAnalysisBUTViewComp(fig_group,ga,bg)
...
set(ui_list_threshold,'String','Select measure to view applicable threshold')
...
%% Callback functions
    ...
    function update_thresholds()
        ...
        if isempty(mi)
            ...
            for i = 1:1:length(ms)
                thresholds(i) = ms{i}.getProp(PETComparisonBUT.THRESHOLD);
            end
        ...
        if strcmp(get(ui_list_threshold,'String'),CALC_NOTIFICATION)
            ...
            if isempty(meas)
                ...
                for i = 1:1:length(ga.getBrainAtlas)
                    p1 = measures{value}.getProp(PETComparisonBUT.PVALUE1);
                    p2 = measures{value}.getProp(PETComparisonBUT.PVALUE2);
                    if get(ui_checkbox_meas_fdr1,'Value')
