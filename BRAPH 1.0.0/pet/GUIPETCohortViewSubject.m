function fig_subject = GUIPETCohortViewSubject(fig_subject,cohort,ba)
...
set(fig_subject,'Name',[APPNAME ' : Subject settings - ' BNC.VERSION ' - ' cohort.getProp(PETCohort.NAME)])
...
set(ui_list,'String',cohort.getProps(PETSubject.CODE))
set(ui_list,'Value',1)
...
update_subject_data()
    ...
    function update_subject_data()
        ...
        if get(ui_checkbox_abs,'Value')
            subject_data = abs(cohort.get(i).getProp(PETSubject.DATA));
        else
            subject_data = cohort.get(i).getProp(PETSubject.DATA);
        end
    end
end