% File to test the use of path length and characteristic path length measures of a weighted directed graph.
%
...
if isequaln(pth_in,pin) && isequaln(pth_out,pout) && isequaln(pth,p)
    disp('Weighted directed path length calculated correctly')
else
    errordlg('Weighted directed path length NOT correct')
end

if isequaln(in_cpth,cpin) && isequaln(out_cpth,cpout) && isequaln(cpth,cp)
    disp('Weighted directed characteristic path length calculated correctly')
else
    errordlg('Weighted directed characteristic path length NOT correct')
end