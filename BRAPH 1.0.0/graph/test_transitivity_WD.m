% File to test the use of transitivity measure of a weighted directed graph.
%
...
for j = 1:1:N
    ...
    for i = 1:1:N
        if A(i,j)~=0
            ...
            text(mean([x(1,i) x(1,j)])-sign(x(1,i)), mean([y(1,i) y(1,j)])+2*sign(y(1,i)),...
                W(i,j),'Interpreter','latex','FontSize',15)
        end
    end
end
axis equal
