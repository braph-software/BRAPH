...
init_table()
    function init_table()
        ...
        set(ui_table_table,'ColumnFormat',{'logical','char','char','numeric','numeric','numeric',BrainRegion.HS_OPTIONS,'char'})
    ...
    function update_table_table()
        ...
        for i = 1:1:atlas.length()
            ...
            data{i,TAB_BR_LABEL_COL} = atlas.get(i).getPropValue(BrainRegion.LABEL);
            data{i,TAB_BR_NAME_COL} = atlas.get(i).getPropValue(BrainRegion.NAME);
            data{i,TAB_BR_X_COL} = atlas.get(i).getPropValue(BrainRegion.X);
            data{i,TAB_BR_Y_COL} = atlas.get(i).getPropValue(BrainRegion.Y);
            data{i,TAB_BR_Z_COL} = atlas.get(i).getPropValue(BrainRegion.Z);
            data{i,TAB_BR_HS_COL} = atlas.get(i).getPropValue(BrainRegion.HS);
            data{i,TAB_BR_NOTES_COL} = atlas.get(i).getPropValue(BrainRegion.NOTES);
        end
        set(ui_table_table,'Data',data)
    end
    function cb_table_atlasname(~,~)  % (src,event)
        atlas.setProp(BrainAtlas.NAME,get(ui_edit_table_atlasname,'String'));
